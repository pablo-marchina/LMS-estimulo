...
async function createFixture(root) {
  ...
  const migrations = [
    {
      ...
      name: 'm13a_fixture_one',
      remoteSql: 'select 1;',
    },
    {
      version: '20260709000002',
      name: 'm13b_fixture_two',
      remoteSql: 'select 2;',
    },
  ];
  ...
  return {
    options: { manifestFile, migrationsDirectory, canonicalFile },
    expected,
    manifest,
  };
}
...
test('accepts a complete deterministic recovered history', async () => {
  ...
  try {
    ...
    const result = await validateRecoveredHistory(
      fixture.options,
      fixture.expected,
    );
...
test('rejects a manifest with the wrong aggregate fingerprint', async () => {
  ...
  try {
    ...
    const manifest = JSON.parse(
      await readFile(fixture.options.manifestFile, 'utf8'),
    );
