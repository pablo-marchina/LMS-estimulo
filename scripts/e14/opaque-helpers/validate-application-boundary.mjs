...
const rpcPath = resolve(repositoryRoot, "apps/web/lib/e14/rpc.ts");
const boundaryPath = resolve(repositoryRoot, "apps/web/lib/e14/legacy-rpc-arguments.ts");
...
for (const absolutePath of applicationFiles) {
  ...
  for (const rpcName of opaqueRpcs) {
    if (source.includes(`"${rpcName}"`) || source.includes(`'${rpcName}'`)) {
      fail(
        `${relative(repositoryRoot, absolutePath)} references ${rpcName} outside apps/web/lib/e14/rpc.ts.`
      );
    }
  }
}
