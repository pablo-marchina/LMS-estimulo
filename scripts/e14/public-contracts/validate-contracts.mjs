...
const manifestPath = path.join(
  repositoryRoot,
  'docs/implementation/e14-public-rpc-contracts-v1.json',
);
const inventorySqlPath = path.join(repositoryRoot, 'scripts/e14/public-contracts/inventory.sql');
...
export function validateApplicationContract(manifest, source) {
  ...
  assert.ok(source.includes('if (error) throw new E14RpcError(error.code ?? "E14_RPC_ERROR", error.message);'), 'database error code propagation changed');
  assert.deepEqual(
    sortedUnique([
...
export async function validatePublicContracts(databaseUrl) {
  ...
  assert.equal(manifest.artifact, 'e14_public_rpc_contracts');
