...
async function assertMissing(relative) {
  await assert.rejects(read(relative), (error) => error?.code === "ENOENT", `${relative} must not exist`);
}
...
test("the repository has one root environment example and no test-signup switch", async () => {
  ...
  assert.match(rootExample, /Supabase and Vercel are restricted to development, test and preview/u);
});
...
test("participant signup uses the public Auth flow and defers protected profile data", async () => {
  ...
  assert.doesNotMatch(action, /auth\.admin\.createUser|email_confirm:\s*true|protectCpf/u);
...
test("runtime identity and RPC calls use the authenticated gateway without a local service-role key", async () => {
  const [currentIdentity, context, sharedGateway, serverInvoke, edgeGateway] = await Promise.all([
