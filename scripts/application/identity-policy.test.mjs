...
const [adminEmailPolicy, adminLayout, signInAction, completionPage, completionAction, migration, hubSpotPolicy] = await Promise.all([
...
test("administrative entry requires the exact Estímulo domain and keeps RBAC", () => {
  ...
  assert.match(adminLayout, /isEstimuloAdministrativeEmail\(auth\.email\)/u);
  assert.match(signInAction, /isEstimuloAdministrativeEmail\(auth\.email\)/u);
  assert.match(signInAction, /permissions\.some/u);
  assert.doesNotMatch(signInAction, /grant|membership_roles|organization_memberships/u);
});
