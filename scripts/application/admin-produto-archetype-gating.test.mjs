...
const contentSection = await readFile(
  "apps/web/app/admin/produto/journey-content-section.tsx",
  "utf8",
);
...
const actions = await readFile(
  "apps/web/app/admin/produto/actions.ts",
  "utf8",
);
const runtime = await readFile(
  "apps/web/lib/admin/product-management.ts",
  "utf8",
);

test("product editor uses structured track and lesson forms through a composition root", () => {
  assert.match(page, /JourneyContentSection/u);
...
test("product actions forward typed payloads through the administrative runtime", () => {
  ...
  assert.match(
    actions,
    /const assessmentWasSubmitted = formData\.has\("quiz_question_count"\)/u,
  );
  ...
  assert.doesNotMatch(
    actions,
    /createPrivilegedClient|SUPABASE_SERVICE_ROLE_KEY/u,
  );
});
