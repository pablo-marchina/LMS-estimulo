...
const [publicationSection, archiveAction, saveAction, lifecycle, nextConfig, bannerStorage] = await Promise.all([
...
test("published journeys expose archive separately from disruptive unpublish", () => {
  ...
  assert.match(publicationSection, /Participantes em andamento serão interrompidos imediatamente/u);
  assert.match(publicationSection, /Arquivar remove a jornada das listas ativas sem apagar o histórico/u);
});
