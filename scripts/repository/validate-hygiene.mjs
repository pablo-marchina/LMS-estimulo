...
const forbiddenCanonicalDocumentNamePatterns = (policy.forbiddenCanonicalDocumentNamePatterns ?? []).map((source) => new RegExp(source, "iu"));
const forbiddenCanonicalDocumentPhrases = policy.forbiddenCanonicalDocumentPhrases ?? [];
...
function validatePaths() {
  for (const file of files) {
    if (forbiddenPrefixes.some((prefix) => file.startsWith(prefix))) errors.push(`development-only path tracked: ${file}`);
    if (forbiddenExactFiles.has(file)) errors.push(`obsolete or development-only file tracked: ${file}`);
    if (/^scripts\/database\/[^/]+\/run\.mjs$/u.test(file)) errors.push(`duplicate database wrapper tracked; use scripts/database/run-gates.mjs: ${file}`);
    if (file.startsWith("scripts/application/") && /(?:regression|homolog|frente|round)/iu.test(path.posix.basename(file))) errors.push(`process-bound application test name tracked: ${file}`);
    if (file === ".tmp" || file.startsWith(".tmp/") || file.includes("/.tmp/")) errors.push(`temporary runtime state tracked: ${file}`);
    if (file === ".secrets" || file.startsWith(".secrets/") || file.includes("/.secrets/")) errors.push(`secret material tracked: ${file}`);
    if (file === "coverage" || file.startsWith("coverage/") || file.includes("/coverage/")) errors.push(`coverage output tracked: ${file}`);
    ...
    if (basename.startsWith(".env") && !allowedEnvironmentExamples.has(file)) errors.push(`environment file tracked outside the approved examples: ${file}`);
    ...
    if (file.startsWith("docs/") && file.endsWith(".md")) {
      ...
      if (forbiddenCanonicalDocumentNamePatterns.some((pattern) => pattern.test(basename))) {
        errors.push(`process-bound canonical document name: ${file}`);
      }
    }
  }
}
...
async function validateRequiredFiles() {
  for (const file of requiredFiles) if (!files.includes(file)) errors.push(`required file missing: ${file}`);
  ...
  if (webPackageJson.scripts?.prebuild !== "node ../../scripts/runtime/validate-production-config.mjs") errors.push("apps/web prebuild must only validate production configuration");
...
function referencedByAnotherFile(candidate, entries) {
  ...
  return entries.some(({ file, content }) => file !== candidate && (content.includes(candidate) || content.includes(basename)));
}
...
for (const { file, content } of textEntries) {
  ...
  if (file.startsWith("docs/") && file.endsWith(".md")) {
    for (const phrase of forbiddenCanonicalDocumentPhrases) {
      if (content.toLocaleLowerCase("pt-BR").includes(phrase.toLocaleLowerCase("pt-BR"))) {
        errors.push(`process-bound phrase in canonical document ${file}: ${phrase}`);
      }
    }
  }
}
...
const unindexedMarkdown = files.filter((file) => file.startsWith("docs/") && file.endsWith(".md")).filter((file) => !indexTargets.has(file)).sort();
for (const file of unindexedMarkdown) errors.push(`canonical document missing from PROJECT_INDEX.md: ${file}`);

const unreferencedDocumentationArtifacts = files.filter((file) => file.startsWith("docs/")).filter((file) => [".json", ".yaml", ".yml", ".csv", ".sql", ".txt"].includes(path.posix.extname(file))).filter((file) => !referencedByAnotherFile(file, textEntries)).sort();
for (const file of unreferencedDocumentationArtifacts) errors.push(`unreferenced documentation artifact: ${file}`);

const unreferencedScripts = files.filter((file) => file.startsWith("scripts/")).filter((file) => [".mjs", ".js", ".py", ".ps1", ".sql"].includes(path.posix.extname(file))).filter((file) => !isKnownGlobEntrypoint(file)).filter((file) => !referencedByAnotherFile(file, textEntries)).sort();
for (const file of unreferencedScripts) errors.push(`unreferenced script: ${file}`);

const result = { status: errors.length === 0 ? "ok" : "failed", files_scanned: files.length, markdown_files: markdownFiles.length, errors };
