...
const policyFile = "scripts/repository/hygiene-policy.mjs";
const errors = [];
const files = [];
...
function validatePaths() {
  for (const file of files) {
    ...
    if (forbiddenExactFiles.has(file)) {
      errors.push(`obsolete or development-only file tracked: ${file}`);
    }

    if (/^scripts\/database\/[^/]+\/run\.mjs$/u.test(file)) {
      errors.push(
        `duplicate database wrapper tracked; use scripts/database/run-gates.mjs: ${file}`,
      );
    }

    if (
      file.startsWith("scripts/application/") &&
      /(?:regression|homolog|frente|round)/iu.test(path.posix.basename(file))
    ) {
    ...
    if (isGeneratedEvidence(file)) {
      errors.push(`generated evidence tracked: ${file}`);
    }

    if (file.includes("/runtime-split/")) {
      errors.push(`duplicate runtime split tracked: ${file}`);
    }

    if (file.startsWith("generated/")) {
      errors.push(`unused generated source tracked: ${file}`);
    }

    if (/\.local(?:\.|$)/i.test(file)) {
      errors.push(`local artifact tracked: ${file}`);
    }

    if (!isApprovedEnvironmentFile(file)) {
      errors.push(
        `environment file tracked outside the approved examples: ${file}`,
      );
    }
    ...
    if (
      ...
      file.endsWith(".md") &&
      !isCanonicalDocumentName(file)
    ) {
      errors.push(`invalid canonical document name: ${file}`);
    }
  }
}
...
async function validateLocalLinks(file) {
  ...
  for (const raw of links) {
    ...
    if (!(await exists(target))) {
      errors.push(`broken local link in ${file}: ${raw}`);
    }
  }
}
...
async function validateRequiredFiles() {
  ...
  const expectedScripts = {
    "validate:repository":
      "node scripts/repository/validate-hygiene.mjs && npm run validate:module-boundaries",
  ...
  if (
    webPackageJson.scripts?.prebuild !==
    "node ../../scripts/runtime/validate-production-config.mjs"
  ) {
    errors.push(
      "apps/web prebuild must only validate production configuration",
    );
  }

  const readme = await readFile(path.join(root, "README.md"), "utf8");
  if (readme.includes("apps/web/.env.local")) {
    errors.push("README.md references obsolete workspace env location");
  }

  const index = await readFile(path.join(root, "PROJECT_INDEX.md"), "utf8");
  if (/#[0-9]+/.test(index)) {
    errors.push("PROJECT_INDEX.md must not depend on transient issue or PR numbers");
  }

  const blockers = await readFile(
    path.join(root, "docs/implementation/DELIVERY_BLOCKERS.md"),
    "utf8",
  );

  if (/#[0-9]+/.test(blockers)) {
    errors.push(
      "DELIVERY_BLOCKERS.md must not depend on transient issue or PR numbers",
    );
  }

  if (/\b[0-9a-f]{40}\b/i.test(blockers)) {
    errors.push("DELIVERY_BLOCKERS.md must not freeze a transient commit SHA");
  }

  if (
    /total_migration_count|active_migration_count|recovered_migration_count/.test(
      blockers,
    )
  ) {
  ...
  if (
    /software_release_candidate_sha|canonical_database_replay\s*=|lambda_container_test_(?:rps|error_rate|p95_ms|p99_ms)/.test(
      blockers,
    )
  ) {
    errors.push(
      "DELIVERY_BLOCKERS.md must not duplicate transient release evidence",
    );
  }
}
...
async function validateEdgeFunctionSources() {
  ...
  for (const entry of await readdir(functionsRoot, { withFileTypes: true })) {
    ...
    for (const required of ["index.ts", "deno.json"]) {
      ...
      if (!(await exists(target))) {
        errors.push(
          `incomplete edge function source: ${path.relative(root, target)}`,
        );
      }
    }
  }
...
for (const { file, content } of textEntries) {
  ...
  for (const reference of forbiddenReferences) {
    if (content.includes(reference)) {
      errors.push(`development-only reference in ${file}: ${reference}`);
    }
  }
}
...
for (const file of unindexedMarkdown) {
  errors.push(`canonical document missing from PROJECT_INDEX.md: ${file}`);
}

const unreferencedDocumentationArtifacts = files
  .filter((file) => file.startsWith("docs/"))
  .filter((file) =>
    [".json", ".yaml", ".yml", ".csv", ".sql", ".txt"].includes(
...
for (const file of unreferencedDocumentationArtifacts) {
  errors.push(`unreferenced documentation artifact: ${file}`);
}

const unreferencedScripts = files
  .filter((file) => file.startsWith("scripts/"))
  .filter((file) =>
    [".mjs", ".js", ".py", ".ps1", ".sql"].includes(
      path.posix.extname(file),
    ),
  )
...
for (const file of unreferencedScripts) {
  errors.push(`unreferenced script: ${file}`);
}
