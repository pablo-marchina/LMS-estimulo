...
try {
  ...
  const signupDestination = page.waitForURL((url) => {
    const path = url.pathname;
    return path === "/cadastro/concluir" || path === "/entrar" || (path === "/cadastro" && url.searchParams.has("erro"));
  }, { timeout: 120_000 });
  await page.getByRole("button", { name: "Criar minha conta" }).click();
  await signupDestination;
  await page.screenshot({ path: `${artifactDir}/01-signup-result.png`, fullPage: true });
  record("signup_submitted", { url: page.url() });

  if (page.url().includes("/cadastro?erro=")) {
    const signupBody = await page.locator("body").innerText();
    throw new Error(`Signup returned application error: ${page.url()} :: ${signupBody.slice(0, 1200)}`);
  }
  ...
  const profileDestination = page.waitForURL((url) => url.pathname === "/empreendedor" || (url.pathname === "/cadastro/concluir" && url.searchParams.has("erro")), { timeout: 120_000 });
  ...
  if (page.url().includes("/cadastro/concluir?erro=")) {
    ...
    throw new Error(`Profile completion returned application error: ${page.url()} :: ${profileBody.slice(0, 1200)}`);
  }
  ...
  const diagnosticDestination = page.waitForURL(/\/empreendedor\/resultado\?.*diagnostico=concluido/, { timeout: 120_000 });
  await submit.click();
  await diagnosticDestination;
