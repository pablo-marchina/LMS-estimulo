...
A camada pública E14 contém 18 RPCs consumidos exclusivamente pelo backend da aplicação em `apps/web/lib/e14/rpc.ts`:
...
O artefato `e14-public-rpc-contracts-v1.json` fixa:
...
A camada TypeScript deve continuar propagando o código de erro PostgreSQL por `E14RpcError.code`. O fallback `E14_RPC_ERROR` só é usado quando o provedor não entrega um código.
