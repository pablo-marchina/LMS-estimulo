...
- fixture, mock e Browser E2E sintético não equivalem a prova real;
...
| `REAL-FULLSTACK-E2E` | testes de banco e harnesses de navegador existem | não há prova navegador → identidade → aplicação → banco → storage → HubSpot no ambiente-alvo | execução integral com contas e dados de teste controlados |
...
| `GITHUB-ACTIONS-AVAILABILITY` | workflows permanentes e comandos locais definidos | runs atuais encerram antes dos steps e não fornecem logs | workflows executam e checks obrigatórios ficam verdes |
...
production_test_bypass = absent
synthetic_browser_mode = localhost_only
...
Números de migrations, RPCs e testes não são copiados manualmente para este documento. Eles são derivados pelos comandos e contratos versionados do repositório.
