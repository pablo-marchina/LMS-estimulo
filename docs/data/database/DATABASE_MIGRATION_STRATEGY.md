...
## 7. Execução por ambiente

A migration é única e vem do repositório. O pipeline executará:

```text
migration lint/static check
→ Supabase efêmero em CI
→ Supabase test/QA
→ Amazon RDS PostgreSQL em AWS staging
→ gate manual/automatizado
→ Amazon RDS PostgreSQL em produção
```

Requisitos:

- alinhar a versão principal do PostgreSQL entre Supabase e RDS;
- manter allowlist de extensões portáveis;
- impedir alterações manuais não versionadas;
- comparar schema pós-migration;
- executar testes de RLS/autorização nos dois ambientes;
- aplicar backfills com idempotência e telemetria;
- testar restauração antes de mudanças destrutivas.

## 8. Compatibilidade Supabase/AWS

Não utilizar no núcleo:

- `auth.uid()` diretamente em policies;
- Edge Functions como única implementação de regra;
- Supabase Realtime como requisito do domínio;
- PostgREST como única API;
- webhooks proprietários como única origem de eventos;
- extensões ausentes no RDS aprovado.

O helper de contexto interno deverá funcionar nos dois ambientes.
