-- Task 2.5: fix hardcoded aggregate_version=1 in save_admin_product_resource's
-- shared epilogue. The literal `1` passed as aggregate_version to
-- app_private.e14_append_event collided with eventing.events' unique index
-- uq_eventing_aggregate_version on (aggregate_type, aggregate_id,
-- aggregate_version) on every second-and-later save of the same resource
-- definition, since aggregate_type=p_resource_type and
-- aggregate_id=v_subject_id (the definition_id) are stable across saves.
-- This blocked "edit already-saved content a second time" for all 8 admin
-- resource types (journey, activity, path_step, rule, diagnostic,
-- point_rule, badge, certificate).
--
-- Fix: compute a real per-aggregate monotonic counter
-- (coalesce(max(aggregate_version),0)+1 scoped to the same aggregate_type +
-- aggregate_id) instead of hardcoding 1. Confirmed safe: nothing reads
-- aggregate_version for this event (get_admin_reporting_dashboard's
-- recent_events selects only event_name, occurred_at, aggregate_type,
-- aggregate_id; no other function references
-- 'admin.product.configuration.saved').
--
-- Every other line of the function (every resource-type branch, the declare
-- block, the idempotency/permission checks) is unchanged from the live body
-- pulled via pg_get_functiondef immediately before this migration was
-- authored -- this migration only adds one declare variable
-- (v_aggregate_version bigint), one select statement right before the
-- epilogue's perform app_private.e14_append_event(...) call, and swaps the
-- literal `1` argument for v_aggregate_version.
...
begin
  ...
  if p_resource_type='journey' then
    ...
    if v_version_id is null then
      ...
      values(gen_random_uuid(),v_definition_id,v_next_version,'draft',btrim(p_payload->>'title'),nullif(btrim(p_payload->>'description'),''),coalesce(p_payload->'configuration','{}'::jsonb),'1',null,null,app_private.e14_request_hash(jsonb_build_object('title',p_payload->>'title','description',p_payload->>'description','configuration',coalesce(p_payload->'configuration','{}'::jsonb))),p_actor_user_account_id,now())
      returning id into v_version_id;
    else
      update catalog.journey_versions set title=btrim(p_payload->>'title'),description=nullif(btrim(p_payload->>'description'),''),configuration=coalesce(p_payload->'configuration','{}'::jsonb),content_hash=app_private.e14_request_hash(jsonb_build_object('title',p_payload->>'title','description',p_payload->>'description','configuration',coalesce(p_payload->'configuration','{}'::jsonb)))
