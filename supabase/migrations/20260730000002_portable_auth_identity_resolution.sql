-- Supabase Postgres images and hosted GoTrue revisions expose the confirmation
-- timestamp under different physical columns. Identity recovery must validate a
-- confirmed user without binding the application migration to one provider-owned
-- table revision. Access through to_jsonb remains fail-closed when neither field
-- exists or when both are null.
  ...
  if v_account_id is not null then
    if not exists (
      ...
      where auth_user.id::text = trim(p_subject)
        ...
        and coalesce(
          nullif(to_jsonb(auth_user)->>'email_confirmed_at','')::timestamptz,
          nullif(to_jsonb(auth_user)->>'confirmed_at','')::timestamptz
        ) is not null
    ...
    if v_identity_id is null then
      if trim(p_provider) <> 'google'
         ...
         or not exists (
           ...
           where current_auth_user.id::text = trim(p_subject)
             ...
             and coalesce(
               nullif(to_jsonb(current_auth_user)->>'email_confirmed_at','')::timestamptz,
               nullif(to_jsonb(current_auth_user)->>'confirmed_at','')::timestamptz
             ) is not null
             and (to_jsonb(current_auth_user)->>'deleted_at') is null
             and (
      ...
      join auth.users previous_auth_user
       ...
       and (to_jsonb(previous_auth_user)->>'deleted_at') is null
      where identity.user_account_id = v_account_id
        and identity.subject <> trim(p_subject);
...
comment on function iam.resolve_external_identity(text,text,text,text,boolean,text) is
  'Resolves a verified external identity with guarded stale-subject recovery across supported Supabase Auth schema revisions.';
