create or replace function public.update_admin_activity_asset_metadata(
...
begin
  ...
  where asset.activity_version_id=p_activity_version_id
    and definition.owner_organization_id=p_organization_id
  order by asset.position,asset.id
  limit 1
  for update of asset;
  if v_asset_id is null then raise exception 'ACTIVITY_ASSET_NOT_FOUND' using errcode='P0002'; end if;
  ...
  v_result:=jsonb_build_object('asset_id',v_asset_id,'activity_version_id',p_activity_version_id,'title',v_title,'description',v_description);
  perform governance.write_audit_entry(
    'admin_activity_asset_metadata_saved','content_asset',v_asset_id,v_result,'internal',p_organization_id,p_actor_user_account_id
  );
  return v_result||jsonb_build_object('idempotency_key',v_key);
end;
$function$;

revoke all on function public.update_admin_activity_asset_metadata(uuid,uuid,uuid,text,text,text) from public,anon,authenticated;
grant execute on function public.update_admin_activity_asset_metadata(uuid,uuid,uuid,text,text,text) to service_role;
