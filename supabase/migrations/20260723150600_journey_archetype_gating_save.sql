-- Task 6: extend save_admin_product_resource's `journey` branch to persist
-- catalog.journey_versions.eligible_archetype_codes on both insert and
-- update. Base is the live pg_get_functiondef pulled immediately before this
-- migration (which already carries doc-comments added to the diagnostic
-- branch and before the lock-scope call since the Task 6 snapshot was taken);
-- every branch other than `journey` is copied verbatim from that live pull.
-- Only the catalog.journey_versions insert/update inside the `journey`
-- branch gained the new column.
