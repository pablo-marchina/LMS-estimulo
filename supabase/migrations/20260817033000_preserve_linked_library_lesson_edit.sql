...
-- Editing lesson metadata must not revalidate/recreate an unchanged library asset.
-- Re-linking is still performed when the selected library version or the asset
-- required flag changes, so intentional content changes keep the existing guards.
...
declare
  ...
  v_base jsonb;
  v_merged jsonb;
  v_existing_library_item_version_id uuid;
  v_requested_library_item_version_id uuid;
  v_existing_content_required boolean;
  v_requested_content_required boolean;
begin
  ...
  select jsonb_build_object(
    ...
    'library_item_version_id', (
      select ca.library_item_version_id
      from catalog.content_assets ca
      where ca.activity_version_id = av.id
      order by ca.position, ca.id
      limit 1
    ),
    'content_required', coalesce((
      select ca.is_required
      from catalog.content_assets ca
      where ca.activity_version_id = av.id
      order by ca.position, ca.id
      limit 1
  ...
  v_merged := v_merged || jsonb_build_object(
    'step_id', v_base->'step_id',
    'path_template_id', v_base->'path_template_id'
  );

  v_existing_library_item_version_id := nullif(v_base->>'library_item_version_id', '')::uuid;
  v_requested_library_item_version_id := nullif(p_payload->>'library_item_version_id', '')::uuid;
  v_existing_content_required := coalesce((v_base->>'content_required')::boolean, false);
  v_requested_content_required := coalesce(
    (p_payload->>'content_required')::boolean,
    v_existing_content_required
  );

  if coalesce(p_payload->>'content_source', '') = 'library'
    and v_existing_library_item_version_id is not null
    and v_requested_library_item_version_id = v_existing_library_item_version_id
    and v_requested_content_required = v_existing_content_required then
    v_merged := v_merged || jsonb_build_object(
      'content_source', 'current',
      'library_item_version_id', v_existing_library_item_version_id,
      'content_required', v_existing_content_required
    );
  end if;

  return public.save_admin_lesson(
