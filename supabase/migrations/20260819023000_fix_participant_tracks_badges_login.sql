...
begin
  ...
  select coalesce((jv.configuration->>'open_all_paths')::boolean,false),e.entrepreneur_id,ji.status
    into v_open_all,v_entrepreneur_id,v_journey_status
  ...
  if v_journey_status not in ('available','in_progress') then
    return jsonb_build_object('journey_instance_id',p_journey_instance_id,'open_all_paths',v_open_all,'reconciled',false);
  end if;

  -- Published tracks are the participant catalog. Reconcile them regardless of
  -- the legacy open_all_paths flag so newly published tracks cannot disappear
  -- for participants who enrolled before the track was created.
  v_result:=public.ensure_participant_default_path(p_actor_user_account_id,p_journey_instance_id,p_idempotency_key);
  return coalesce(v_result,'{}'::jsonb)||jsonb_build_object('open_all_paths',v_open_all,'reconciled',true);
end;
$function$;

-- Retired tracks are historical catalog records, not participant-visible paths.
-- Keep assignment rows for auditability while removing them from the active
-- participant projection.
update orchestration.path_assignments assignment
set status='retired',
    valid_until=case
      when assignment.valid_until is null then greatest(now(), assignment.valid_from + interval '1 microsecond')
      else assignment.valid_until
    end
from orchestration.path_templates template
where template.id=assignment.path_template_id
  and template.status='retired'
  and assignment.status='active';

-- Archiving is no longer a product capability. Track lifecycle remains controlled
-- by the journey editor/publishing flow, while historical retired rows are kept.
drop function if exists public.archive_admin_track(uuid,uuid,uuid,text);
