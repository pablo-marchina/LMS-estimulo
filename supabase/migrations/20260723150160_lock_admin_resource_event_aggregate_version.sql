-- Fix: close read-then-write race in public.save_admin_product_resource's shared
-- epilogue. The select coalesce(max(aggregate_version),0)+1 ... from eventing.events
-- (scoped to aggregate_type=p_resource_type, aggregate_id=v_subject_id) followed by an
-- insert via app_private.e14_append_event is racy under concurrent saves of the same
-- resource (two admin tabs, or a retry racing an in-flight request): both can read the
-- same max and both attempt the same next aggregate_version, and the loser rolls back on
-- eventing.events' uq_eventing_aggregate_version unique constraint.
--
-- Precedent for the identical problem already exists in this codebase:
-- app_private.library_event_aggregate_version_trigger takes
-- pg_advisory_xact_lock(hashtextextended(...)) immediately before computing the same
-- coalesce(max(aggregate_version),0)+1 for eventing.events. app_private.e14_lock_scope is
-- the established generic wrapper for that same call
-- (`select pg_advisory_xact_lock(hashtextextended(p_scope,0))`), already used throughout
-- this function's own E14 command family (e14_prepare_a etc. call it with pipe-delimited
-- composite scope keys) even though save_admin_product_resource itself didn't need a lock
-- before now. Using the helper here is the smaller, more obviously-correct change, and it
-- is transaction-scoped (pg_advisory_xact_lock), so it releases automatically at commit
-- or rollback -- no explicit unlock required.
