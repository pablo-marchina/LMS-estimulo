-- Task 3: let any "aula" (activity) carry an admin-editable quiz.
--
-- Adds an `assessment` block to the `activity` branch of
-- save_admin_product_resource(): when p_payload.assessment is present, the
-- activity_version's assessment.assessment_specs row and all its
-- questions/answer_options are rebuilt (existing questions/options deleted
-- and reinserted), mirroring Frente 3+4 Task 2's diagnostic child rebuild
-- pattern. This is independent of activity_type -- any activity can carry
-- an assessment.
--
-- Every other branch of this function is carried over byte-identical from
-- the live definition (as changed by Task 2 / path_template work) except for
-- the two new declare entries and this one new block inserted into the
-- `activity` branch immediately after the existing practice-spec block.
--
-- Deviation from the plan's Step 2 snippet: assessment.assessment_specs has
-- NO `id` column -- activity_version_id is the table's primary key (verified
-- live via list_tables). The insert/on-conflict/returning clause below is
-- adjusted accordingly (no `id` column in the column list or values list;
-- `returning activity_version_id into v_assessment_spec_id` instead of
-- `returning id`).
--
-- Draft/publish note: the `activity` branch of this function never flips
-- catalog.activity_versions.status to 'published' -- rows are created as
-- 'draft' and updates require status='draft'. The deferred-publish step for
-- activities lives in a separate function, public.e14_publish_vertical
-- (confirmed live), which does
-- `update catalog.activity_versions set status='published' ... where id=v_activity_id`
-- as part of publishing the whole journey vertical. That means the
-- assessment rebuild below always runs against a draft row through this RPC,
-- so it can never trip the trg_e14_assessment_*_immutable triggers
-- (app_private.e14_reject_published_assessment_child), which only raise
-- PUBLISHED_ASSESSMENT_IMMUTABLE once the parent activity_version is
-- 'published'.
...
begin
  ...
  if p_resource_type='journey' then
    ...
    if p_payload?'assessment' and jsonb_typeof(p_payload->'assessment')='object' then
      ...
      values(v_activity_version_id,'auto',
      ...
      returning activity_version_id into v_assessment_spec_id;
