create or replace function public.ensure_participant_default_path(
  p_actor_user_account_id uuid,p_journey_instance_id uuid,p_idempotency_key text
...
declare
  ...
  v_entrepreneur_id uuid; v_journey_version_id uuid; v_organization_id uuid; v_journey_status text;
  v_path_template_id uuid; v_path_code text; v_assignment_id uuid;
  v_first_path_step_id uuid; v_first_step_instance_id uuid; v_total_required integer;
  ...
  v_started_event_id uuid; v_step_event_id uuid; v_result jsonb;
begin
  ...
  from orchestration.journey_instances instance join orchestration.enrollments enrollment on enrollment.id=instance.enrollment_id
  ...
  select assignment.path_template_id,template.code,assignment.id into v_path_template_id,v_path_code,v_assignment_id
  from orchestration.path_assignments assignment join orchestration.path_templates template on template.id=assignment.path_template_id
  ...
  if v_path_template_id is null then
    ...
    values(v_assignment_id,p_journey_instance_id,v_path_template_id,null,'active',jsonb_build_object('source','participant_optional_diagnostic_skip','idempotency_key',v_key),1,now())
    on conflict(id) do nothing;
  end if;

  select step.id into v_first_path_step_id from orchestration.path_steps step
  where step.path_template_id=v_path_template_id order by step.position_hint,step.id limit 1;
  ...
  select count(*) filter(where step.is_required) into v_total_required from orchestration.path_steps step where step.path_template_id=v_path_template_id;
  update orchestration.progress_projections set total_required_steps=greatest(coalesce(v_total_required,0),1),
    current_step_id=coalesce(current_step_id,v_first_path_step_id),last_activity_at=coalesce(last_activity_at,now()),
    projection_version=projection_version+1,updated_at=now() where journey_instance_id=p_journey_instance_id;

  if not exists(select 1 from eventing.events event where event.event_id=v_event_id) then
    perform app_private.e14_append_event(v_event_id,'journey.path.assigned','path_assignment',v_assignment_id,'user_account',p_actor_user_account_id,v_organization_id,p_journey_instance_id,'path_assignment',v_assignment_id,0,v_event_id,null,jsonb_build_object('path_code',v_path_code,'source','participant_optional_diagnostic_skip'));
    v_started_event_id:=app_private.e14_child_event_id(v_event_id,'journey.path.started',1);
    perform app_private.e14_append_event(v_started_event_id,'journey.path.started','path_assignment',v_assignment_id,'user_account',p_actor_user_account_id,v_organization_id,p_journey_instance_id,'path_assignment',v_assignment_id,1,v_event_id,v_event_id,jsonb_build_object('path_code',v_path_code));
    v_step_event_id:=app_private.e14_child_event_id(v_event_id,'journey.step.available',2);
    perform app_private.e14_append_event(v_step_event_id,'journey.step.available','step_instance',v_first_step_instance_id,'user_account',p_actor_user_account_id,v_organization_id,p_journey_instance_id,'step_instance',v_first_step_instance_id,0,v_event_id,v_started_event_id,jsonb_build_object('path_code',v_path_code,'path_step_id',v_first_path_step_id));
  end if;
  v_result:=jsonb_build_object('journey_instance_id',p_journey_instance_id,'path_assignment_id',v_assignment_id,'path_template_id',v_path_template_id,'path_code',v_path_code,'first_step_instance_id',v_first_step_instance_id,'step_count',(select count(*) from orchestration.step_instances where path_assignment_id=v_assignment_id));
