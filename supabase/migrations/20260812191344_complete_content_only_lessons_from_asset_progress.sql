...
begin
  select assignment.journey_instance_id,step.path_step_id,step.activity_version_id,definition.owner_organization_id
    into v_journey_instance_id,v_path_step_id,v_activity_version_id,v_organization_id
  ...
  v_ratio:=case
    ...
    when coalesce(p_duration_seconds,0)>0 then least(1,greatest(0,coalesce(p_position_seconds,0)/p_duration_seconds))
    else 0
  end;
  ...
  insert into orchestration.activity_asset_progress(
    ...
    p_step_instance_id,p_content_asset_id,greatest(0,coalesce(p_position_seconds,0)),nullif(p_duration_seconds,0),v_ratio,
  ...
  on conflict(step_instance_id,content_asset_id) do update set
    ...
    duration_seconds=coalesce(excluded.duration_seconds,orchestration.activity_asset_progress.duration_seconds),
