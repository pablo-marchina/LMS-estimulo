...
-- Backfill a read projection for the current admin workspace. The canonical
-- relationship remains engagement.path_badge_links.
update orchestration.path_templates path
set presentation=coalesce(path.presentation,'{}'::jsonb)||jsonb_build_object(
  'completion_badge_version_id',link.badge_version_id
)
from engagement.path_badge_links link
where link.path_template_id=path.id;
