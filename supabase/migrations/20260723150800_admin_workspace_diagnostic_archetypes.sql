-- Extend get_admin_product_workspace's diagnostics branch (again) to also
-- return `archetypes` per diagnostic version, closing the read gap that
-- causes the admin diagnostic editor's archetype-description textareas to
-- render blank on edit-load. archetype_definitions/archetype_versions are
-- org-scoped tables linked to a diagnostic version via
-- archetype_versions.model_reference = diagnostic_version_id::text; this
-- join is additive alongside the existing dimensions/items sub-selects and
-- scoped to only the archetypes actually linked to this specific version
-- (not all archetypes in the org).
  ...
  return jsonb_build_object(
    ...
    'diagnostics',(select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'versions',(select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'archetypes',(select coalesce(jsonb_agg(jsonb_build_object('code',ad2.code,'name',ad2.name,'description',ad2.description) order by ad2.name),'[]'::jsonb) from diagnostics.archetype_versions av2 join diagnostics.archetype_definitions ad2 on ad2.id=av2.archetype_definition_id where av2.model_reference=dv.id::text and ad2.owner_organization_id=p_organization_id)
