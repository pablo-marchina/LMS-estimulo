-- Task 2: diagnostic publish flow + classification rule storage.
-- Rebuilds public.save_admin_product_resource, replacing ONLY the 'diagnostic'
-- branch (adds status/publish support + classification_rules storage on
-- diagnostics.diagnostic_versions.configuration, and publish support on
-- diagnostics.archetype_versions). All other branches (journey/activity/
-- path_step/rule/point_rule/badge/certificate) are copied verbatim from the
-- live function body (confirmed via pg_get_functiondef against the live DB,
-- not from the prior migration file) to avoid drifting from production
-- behavior.
--
-- Schema fix required for the new logic to work: diagnostics.diagnostic_versions
-- .published_at was NOT NULL in the live schema (unlike the sibling
-- archetype_versions/segment_versions tables, which are nullable). The
-- pre-existing 'diagnostic' branch worked around this by unconditionally
-- setting published_at=now() on version creation, even for drafts -- which
-- left at least one live row (business_maturity_self_assessment, version 1)
-- with status='draft' but a non-null published_at equal to its created_at.
-- That legacy row is left untouched here (not in scope for this migration,
-- and explicitly excluded as a test fixture); new/updated versions after this
-- migration get correct null published_at while in draft.
alter table diagnostics.diagnostic_versions alter column published_at drop not null;
