...
begin
  ...
  if exists(
    ...
    raise exception 'REQUIRED_CONTENT_INCOMPLETE' using errcode='55000';
  end if;
  ...
  then
    raise exception 'ASSESSMENT_NOT_PASSED' using errcode='55000';
  end if;

  -- Practice completion remains owned by the review workflow. The participant
  -- CTA never bypasses an evidence/review requirement.
  if exists(select 1 from assessment.practice_specs practice where practice.activity_version_id=v_activity_version_id) then
    raise exception 'PRACTICE_COMPLETION_MANAGED_BY_REVIEW' using errcode='55000';
  end if;
...
revoke all on function public.complete_participant_activity(uuid,uuid,text) from public,anon,authenticated;
grant execute on function public.complete_participant_activity(uuid,uuid,text) to service_role,app_worker;
