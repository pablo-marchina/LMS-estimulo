create table if not exists experience.admin_content_revisions(
...
alter table experience.admin_content_revisions enable row level security;
revoke all on experience.admin_content_revisions from public,anon,authenticated;
grant select,insert on experience.admin_content_revisions to service_role;
create index if not exists ix_experience_admin_content_revisions_resource
  on experience.admin_content_revisions(organization_id,resource_type,resource_id,created_at desc);

with names(event_name) as (
  values('admin.journey.saved'::text),('admin.lesson.saved'::text)
),docs as (
...
begin
  ...
  if v_version_id is null then
    if v_definition_id is null then
      ...
      update catalog.journey_definitions set
        program_id=coalesce(nullif(p_payload->>'program_id','')::uuid,program_id),
        code=v_code,
    ...
    insert into catalog.journey_versions(
      ...
      v_version_id,v_definition_id,v_next,'draft',btrim(p_payload->>'title'),nullif(btrim(p_payload->>'description'),''),
      v_configuration,'1',(select array_agg(value) from jsonb_array_elements_text(coalesce(p_payload->'eligible_archetype_codes','[]'::jsonb)) value),
      null,null,app_private.e14_request_hash(jsonb_build_object(
    ...
    where jv.id=v_version_id and jd.owner_organization_id=p_organization_id and jv.status in ('draft','published')
    for update of jv,jd;
    if not found then raise exception 'JOURNEY_NOT_FOUND' using errcode='P0002'; end if;
    if v_status='published' and not app_private.e14_actor_has_permission(p_actor_user_account_id,p_organization_id,'journey.definition.publish') then raise exception 'FORBIDDEN_PUBLISH' using errcode='42501'; end if;
    if v_status='published' then perform set_config('app.admin_live_edit','on',true); end if;
    update catalog.journey_definitions set
      program_id=coalesce(nullif(p_payload->>'program_id','')::uuid,program_id),code=v_code,
      slug=lower(btrim(coalesce(nullif(p_payload->>'slug',''),slug))),
      name=btrim(coalesce(nullif(p_payload->>'name',''),p_payload->>'title')),
      purpose=nullif(btrim(p_payload->>'purpose'),''),updated_at=now()
    where id=v_definition_id and owner_organization_id=p_organization_id;
    update catalog.journey_versions set
      title=btrim(p_payload->>'title'),description=nullif(btrim(p_payload->>'description'),''),configuration=v_configuration,
  ...
  select jsonb_build_object('journey_version',to_jsonb(jv),'journey_definition',to_jsonb(jd)) into v_after
  from catalog.journey_versions jv join catalog.journey_definitions jd on jd.id=jv.journey_definition_id where jv.id=v_version_id;
  ...
  values(p_organization_id,'journey_version',v_version_id,
    case when v_before is null then 'created' when v_status='published' then 'live_updated' else 'draft_updated' end,
