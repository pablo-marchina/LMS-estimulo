...
create or replace function app_private.behavior_score_interaction_type(
  p_event_name text,
  p_payload jsonb
)
returns text
...
as $function$
  select case
    when p_event_name='behavior.interaction.recorded' then nullif(p_payload->>'interaction_type','')
    when p_event_name='learning.activity.started' then 'content_view'
    when p_event_name='learning.activity.progressed' then 'content_view'
    when p_event_name='learning.activity.completed' then 'activity_complete'
    when p_event_name='learning.library_content.accessed' then 'library_open'
    when p_event_name='learning.activity.comment.created' then 'discussion_contribution'
    when p_event_name='learning.activity.utility.rated' then 'feedback_submit'
    when p_event_name='assessment.attempt.submitted' then 'delivery_submit'
    when p_event_name='diagnostic.session.completed' then 'diagnostic_complete'
    when p_event_name='journey.path.completed' then 'path_complete'
    when p_event_name='journey.instance.completed' then 'journey_complete'
    when p_event_name='learning.external_credential.confirmed' then 'external_credential_complete'
    else null
  end
$function$;
  ...
  for rec in
    ...
    select
      ...
      coalesce((
        ...
        where submission.entrepreneur_id=entrepreneur.id and submission.final_score is not null
      ),0)::numeric as quality_average
    from core.entrepreneurs entrepreneur
    left join scored_events scored_event on scored_event.actor_id=entrepreneur.user_account_id
    where entrepreneur.status='active' and (p_entrepreneur_id is null or entrepreneur.id=p_entrepreneur_id)
    group by entrepreneur.id,entrepreneur.user_account_id
  loop
    v_dimensions:='{}'::jsonb;
    v_weighted_sum:=0;
    v_weight_total:=0;
...
begin
  if new.organization_id is null
    ...
    or new.event_name='behavior.interaction.recorded'
...
drop trigger if exists trg_recalculate_behavior_score_after_domain_event on eventing.events;
create trigger trg_recalculate_behavior_score_after_domain_event
after insert on eventing.events
for each row
execute function app_private.recalculate_behavior_score_after_domain_event();

revoke all on function app_private.behavior_score_interaction_type(text,jsonb) from public,anon,authenticated;
revoke all on function app_private.recalculate_behavior_score_after_domain_event() from public,anon,authenticated;
grant execute on function app_private.behavior_score_interaction_type(text,jsonb) to service_role;
...
begin
  for organization in
    select distinct score_definition.owner_organization_id
    from intelligence.score_definitions score_definition
    join intelligence.score_versions score_version on score_version.score_definition_id=score_definition.id
    where score_definition.code='behavioral_engagement_v1' and score_version.status='published'
  loop
    perform app_private.recalculate_behavior_scores(organization.owner_organization_id,null);
  end loop;
end;
$backfill$;
