...
-- This mapping is administrative configuration. Runtime access goes through
-- SECURITY DEFINER functions; clients and app_worker do not need direct table access.
...
comment on table engagement.path_badge_links is
  'Administrative mapping between a path template and the published badge awarded when that path is completed. Direct client and app_worker access is denied; runtime access is mediated by security-definer functions.';
    ...
    for v_record in
      with path_candidates as (
        ...
        select distinct on (id)
          id,title,description,criteria_rule_version_id,priority
        from path_candidates
        order by id,priority
      )
      select id,title,description,criteria_rule_version_id
      from deduplicated
      order by id
    loop
      v_award_id:=app_private.e14_deterministic_uuid(
