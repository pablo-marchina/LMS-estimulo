...
select
  'engagement.social.shared',1,'urn:estimulo:event:engagement.social.shared:1',v.document,
  encode(extensions.digest(convert_to(v.document::text,'UTF8'),'sha256'),'hex'),'published',now()
...
where not exists (select 1 from eventing.event_schemas s where s.event_name='engagement.social.shared' and s.event_version=1);
...
declare
  ...
  v_result jsonb;
begin
  if not exists(select 1 from iam.user_accounts u where u.id=p_actor_user_account_id and u.status='active') then raise exception 'ACTOR_NOT_FOUND' using errcode='P0002'; end if;
  ...
  select id into v_social_schema_id from eventing.event_schemas where event_name='engagement.social.shared' and event_version=1 and status='published';
  select id into v_behavior_schema_id from eventing.event_schemas where event_name='behavior.interaction.recorded' and event_version=1 and status='published';
  ...
  perform eventing.append_event(
    ...
    nullif(p_payload->>'journey_instance_id','')::uuid,'behavior_interaction',v_behavior_event_id,0,
    ...
    jsonb_build_object(
      ...
      'session_id',nullif(p_payload->>'session_id',''),'entity_type',v_entity_type,'entity_id',v_entity_id,
      'properties',coalesce(p_payload->'properties','{}'::jsonb)||jsonb_build_object('channel',v_channel)
  ...
  perform eventing.append_event(
    ...
    'participant_web','entrepreneur',v_entrepreneur_id,'user_account',p_actor_user_account_id,v_organization_id,
    nullif(p_payload->>'journey_instance_id','')::uuid,'social_share',v_event_id,0,
...
as $$
begin
  if p_action='social_share' then return app_private.record_participant_social_share(p_actor_user_account_id,p_payload,p_idempotency_key); end if;
  return public.perform_participant_extension_before_continuous_behavior_score(p_actor_user_account_id,p_action,p_payload,p_idempotency_key);
end;
$$;
