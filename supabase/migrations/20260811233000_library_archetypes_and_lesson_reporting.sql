...
begin
  ...
  return jsonb_build_object(
    ...
    'metrics',jsonb_build_object(
      'participants',(select count(distinct enrollment.entrepreneur_id)
        ...
        where definition.owner_organization_id=p_organization_id and definition.status='active'
          and definition.code not in ('e14_runtime_validation_journey','task4_verify_journey')
          and enrollment.status in ('assigned','accepted','active','completed')),
      'enrollments',(select count(*)
        from orchestration.enrollments enrollment
        join core.entrepreneurs entrepreneur on entrepreneur.id=enrollment.entrepreneur_id and entrepreneur.status='active'
        join catalog.journey_versions version on version.id=enrollment.journey_version_id
        join catalog.journey_definitions definition on definition.id=version.journey_definition_id
        where definition.owner_organization_id=p_organization_id and definition.status='active'
          and definition.code not in ('e14_runtime_validation_journey','task4_verify_journey')
          and enrollment.status in ('assigned','accepted','active','completed')),
      'completed_journeys',(select count(*)
      ...
      'average_progress',(select coalesce(round(avg(progress.completion_ratio*100)::numeric,2),0)
        from orchestration.progress_projections progress
        join orchestration.journey_instances instance on instance.id=progress.journey_instance_id
        join orchestration.enrollments enrollment on enrollment.id=instance.enrollment_id
        join core.entrepreneurs entrepreneur on entrepreneur.id=enrollment.entrepreneur_id and entrepreneur.status='active'
        join catalog.journey_versions version on version.id=enrollment.journey_version_id
        join catalog.journey_definitions definition on definition.id=version.journey_definition_id
        where definition.owner_organization_id=p_organization_id and definition.status='active'
          and definition.code not in ('e14_runtime_validation_journey','task4_verify_journey')),
      'points_issued',(select coalesce(sum(ledger.amount),0)
        from engagement.point_ledger ledger
        join core.entrepreneurs entrepreneur on entrepreneur.id=ledger.entrepreneur_id and entrepreneur.status='active'
        join orchestration.journey_instances instance on instance.id=ledger.journey_instance_id
        join orchestration.enrollments enrollment on enrollment.id=instance.enrollment_id
        join catalog.journey_versions version on version.id=enrollment.journey_version_id
        join catalog.journey_definitions definition on definition.id=version.journey_definition_id
        where definition.owner_organization_id=p_organization_id and definition.status='active'
          and definition.code not in ('e14_runtime_validation_journey','task4_verify_journey')),
      'comments',(select count(*) from engagement.activity_comments comment
        join orchestration.journey_instances instance on instance.id=comment.journey_instance_id
        join orchestration.enrollments enrollment on enrollment.id=instance.enrollment_id
        join catalog.journey_versions version on version.id=enrollment.journey_version_id
        join catalog.journey_definitions definition on definition.id=version.journey_definition_id
        where comment.organization_id=p_organization_id and definition.status='active'
          and definition.code not in ('e14_runtime_validation_journey','task4_verify_journey')),
      'practice_submissions',(select count(*)
        from assessment.submissions submission
        join core.entrepreneurs entrepreneur on entrepreneur.id=submission.entrepreneur_id and entrepreneur.status='active'
        join orchestration.step_instances step on step.id=submission.step_instance_id
        ...
        where definition.owner_organization_id=p_organization_id and definition.status='active'
          and definition.code not in ('e14_runtime_validation_journey','task4_verify_journey')),
      'average_utility_rating',(select coalesce(round(avg(rating.rating)::numeric,2),0)
        from engagement.activity_utility_ratings rating
        join orchestration.journey_instances instance on instance.id=rating.journey_instance_id
        join orchestration.enrollments enrollment on enrollment.id=instance.enrollment_id
        join catalog.journey_versions version on version.id=enrollment.journey_version_id
        join catalog.journey_definitions definition on definition.id=version.journey_definition_id
        where rating.organization_id=p_organization_id and definition.status='active'
          and definition.code not in ('e14_runtime_validation_journey','task4_verify_journey')),
      'badges_awarded',(select count(*)
        from engagement.badge_awards award
        join core.entrepreneurs entrepreneur on entrepreneur.id=award.entrepreneur_id and entrepreneur.status='active'
        join engagement.badge_versions badge_version on badge_version.id=award.badge_version_id
        join engagement.badge_definitions badge_definition on badge_definition.id=badge_version.badge_definition_id
        where badge_definition.owner_organization_id=p_organization_id and award.revoked_at is null),
      'certificates_issued',(select count(*)
        from engagement.certificate_issuances issuance
        join core.entrepreneurs entrepreneur on entrepreneur.id=issuance.entrepreneur_id and entrepreneur.status='active'
        ...
        where certificate_definition.owner_organization_id=p_organization_id and issuance.status='issued')
    ),
    'journeys',(select coalesce(jsonb_agg(jsonb_build_object(
      'journey',definition.name,'version',version.version_number,'enrollments',summary.enrollments,
      'completed',summary.completed,'average_progress',summary.average_progress
    ) order by definition.name,version.version_number),'[]'::jsonb)
      from (
        select enrollment.journey_version_id,count(*) enrollments,
          count(*) filter(where instance.status='completed') completed,
          coalesce(round(avg(progress.completion_ratio*100)::numeric,2),0) average_progress
        from orchestration.enrollments enrollment
        join core.entrepreneurs entrepreneur on entrepreneur.id=enrollment.entrepreneur_id and entrepreneur.status='active'
        left join orchestration.journey_instances instance on instance.enrollment_id=enrollment.id
        left join orchestration.progress_projections progress on progress.journey_instance_id=instance.id
        where enrollment.status in ('assigned','accepted','active','completed')
        group by enrollment.journey_version_id
      ) summary
      join catalog.journey_versions version on version.id=summary.journey_version_id
      join catalog.journey_definitions definition on definition.id=version.journey_definition_id
      where definition.owner_organization_id=p_organization_id and definition.status='active'
        and definition.code not in ('e14_runtime_validation_journey','task4_verify_journey')),
    'lessons',(select coalesce(jsonb_agg(jsonb_build_object(
      ...
      from (
        select
          ...
          case when count(step_instance.id)=0 then 0
            else round(
