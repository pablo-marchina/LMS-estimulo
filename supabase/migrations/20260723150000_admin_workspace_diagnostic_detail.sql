-- Task 1 (archetype diagnostic plan): extend the admin product workspace read RPC so it
-- returns full diagnostic content (dimensions/items/options), enabling load-and-edit of
-- existing diagnostic versions instead of only ever creating a fresh draft.
--
-- Also adds `catalog.journey_versions.eligible_archetype_codes`, a column consumed by this
-- same function's `journeys` branch that a later task (archetype gating) will populate.
-- Added here, ahead of that task, because the function body already needs to select it.

alter table catalog.journey_versions
  add column if not exists eligible_archetype_codes text[];
