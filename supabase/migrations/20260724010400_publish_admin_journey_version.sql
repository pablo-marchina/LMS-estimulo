...
begin
  ...
  if exists (
    ...
    where pt.journey_version_id = p_journey_version_id
      ...
      and not exists (
        select 1 from assessment.practice_specs practice where practice.activity_version_id = av.id
      )
  ) then
...
revoke all on function public.publish_admin_journey_version(uuid, uuid, uuid, text, text) from public;
grant execute on function public.publish_admin_journey_version(uuid, uuid, uuid, text, text) to authenticated, service_role;

comment on function public.publish_admin_journey_version(uuid, uuid, uuid, text, text) is
'Publishes a validated admin-authored jornada graph (trilhas, aulas, assessments, practices, path-scoped credential rules and badges) with permission, concurrency and idempotency checks.';
