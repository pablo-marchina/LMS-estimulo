...
begin
  ...
  if p_journey_instance_id is not null then
    select definition.owner_organization_id into v_org
    ...
    where instance.id=p_journey_instance_id and enrollment.entrepreneur_id=v_ent;
    if v_org is null then raise exception 'JOURNEY_INSTANCE_NOT_AVAILABLE' using errcode='P0002'; end if;
  else
    select id into v_org from iam.organizations where slug='estimulo' and status='active' limit 1;
  end if;
  select version.id,version.amount into v_rule,v_amount
  from engagement.point_rule_definitions definition join engagement.point_rule_versions version on version.point_rule_definition_id=definition.id
  where definition.owner_organization_id=v_org and definition.code=p_action_code and definition.status='active' and version.status='published'
  ...
  perform app_private.e14_append_event(
    v_event,'engagement.points.awarded','point_ledger',v_ledger,'user_account',p_actor_user_account_id,
    v_org,p_journey_instance_id,'point_ledger',v_ledger,1,v_event,null,
    jsonb_build_object('code',p_action_code,'amount',v_amount,'source_reference',p_source_reference,'entrepreneur_id',v_ent)
  );
  ...
  select id into v_projection from engagement.point_balance_projections
  where entrepreneur_id=v_ent and journey_instance_id is not distinct from p_journey_instance_id order by updated_at desc limit 1;
  if v_projection is null then
    ...
    update engagement.point_balance_projections set balance=balance+v_amount,last_ledger_entry_id=v_ledger,
      projection_version=projection_version+1,updated_at=now() where id=v_projection;
  end if;
