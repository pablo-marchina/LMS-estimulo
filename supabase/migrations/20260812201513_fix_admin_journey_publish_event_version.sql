...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended('publish_admin_journey_version|' || p_journey_version_id::text, 0));
  ...
  select * into v_journey from catalog.journey_versions where id = p_journey_version_id for update;
  if not found then raise exception 'JOURNEY_NOT_FOUND' using errcode = 'P0002'; end if;
  select owner_organization_id into v_owner_organization_id from catalog.journey_definitions where id = v_journey.journey_definition_id;
  ...
  select count(*) into v_path_count from orchestration.path_templates pt where pt.journey_version_id = p_journey_version_id and pt.status = 'draft';
  select count(*) into v_step_count from orchestration.path_steps ps join orchestration.path_templates pt on pt.id = ps.path_template_id where pt.journey_version_id = p_journey_version_id;
  ...
  if exists (
    select 1 from orchestration.path_templates pt
    where pt.journey_version_id = p_journey_version_id
      and not exists (select 1 from orchestration.path_steps ps where ps.path_template_id = pt.id)
  ) then raise exception 'EMPTY_PATH_TEMPLATE' using errcode = 'P0001'; end if;
  if exists (
    select 1 from orchestration.path_templates pt
    where pt.journey_version_id = p_journey_version_id
    group by pt.position having count(*) > 1
  ) then raise exception 'DUPLICATE_PATH_POSITION' using errcode = 'P0001'; end if;
  if exists (
    select 1 from orchestration.path_steps ps
    join orchestration.path_templates pt on pt.id = ps.path_template_id
    where pt.journey_version_id = p_journey_version_id
    group by ps.path_template_id, ps.position_hint having count(*) > 1
  ) then raise exception 'DUPLICATE_STEP_POSITION' using errcode = 'P0001'; end if;
  if exists (
    select 1 from assessment.questions q
    join orchestration.path_steps ps on ps.activity_version_id = q.activity_version_id
    join orchestration.path_templates pt on pt.id = ps.path_template_id
    where pt.journey_version_id = p_journey_version_id
      and ((select count(*) from assessment.answer_options ao where ao.question_id=q.id)<2
        or (select count(*) from assessment.answer_options ao where ao.question_id=q.id and ao.is_correct)<>1)
  ) then raise exception 'ASSESSMENT_OPTIONS_INVALID' using errcode = 'P0001'; end if;
  if exists (
    select 1 from catalog.activity_versions av
    join orchestration.path_steps ps on ps.activity_version_id = av.id
    join orchestration.path_templates pt on pt.id = ps.path_template_id
    where pt.journey_version_id = p_journey_version_id
      and av.activity_type = 'practice'
      and not exists (select 1 from assessment.practice_specs practice where practice.activity_version_id=av.id)
  ) then raise exception 'PRACTICE_SPEC_REQUIRED' using errcode = 'P0001'; end if;
  update catalog.activity_versions av
     set status='published', published_at=coalesce(av.published_at,now())
   where av.status='draft' and av.id in (
     select ps.activity_version_id from orchestration.path_steps ps
     join orchestration.path_templates pt on pt.id=ps.path_template_id
     where pt.journey_version_id=p_journey_version_id
   );
  get diagnostics v_published_activity_count = row_count;
  update orchestration.rule_versions rv
     set status='published', published_at=coalesce(rv.published_at,now())
   where rv.status='draft' and rv.language='credential-v1'
     and rv.expression->>'scope'='path'
     and rv.expression->>'path_template_id' in (
       select pt.id::text from orchestration.path_templates pt where pt.journey_version_id=p_journey_version_id
     )
     and exists (
       select 1 from engagement.badge_versions bv
       join engagement.badge_definitions bd on bd.id=bv.badge_definition_id
       where bv.criteria_rule_version_id=rv.id and bd.owner_organization_id=p_organization_id
     );
  get diagnostics v_rule_count = row_count;
  update engagement.badge_versions bv
     set status='published', published_at=coalesce(bv.published_at,now())
   where bv.status='draft' and bv.criteria_rule_version_id in (
     select rv.id from orchestration.rule_versions rv
  ...
  update orchestration.path_templates set status='published' where journey_version_id=p_journey_version_id and status='draft';
  ...
  perform app_private.e14_append_event(
    v_event_id,'catalog.journey_version.published','admin_journey_publish',p_journey_version_id,
    'user_account',p_actor_user_account_id,p_organization_id,null,'journey_version',p_journey_version_id,
    v_aggregate_version,v_event_id,null,jsonb_build_object(
      'request_hash',v_request_hash,'idempotency_key',v_key,'content_hash',v_journey.content_hash,
      'path_count',v_path_count,'step_count',v_step_count,
      'published_activity_count',v_published_activity_count,
      'published_rule_count',v_rule_count,'published_badge_count',v_badge_count
    )
  );
  return jsonb_build_object(
    'request_id',v_event_id,'idempotency_key',v_key,'replayed',false,
    'data',jsonb_build_object(
      'journey_version_id',v_journey.id,'status',v_journey.status,
      'published_at',v_journey.published_at,'content_hash',v_journey.content_hash,
      'path_count',v_path_count,'step_count',v_step_count
    )
  );
end;
