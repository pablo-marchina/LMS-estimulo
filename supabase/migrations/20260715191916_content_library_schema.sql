...
create index if not exists library_item_versions_search_idx on catalog.library_item_versions using gin(search_document);
create index if not exists library_item_versions_published_idx on catalog.library_item_versions(status, published_at desc) where status = 'published';
create index if not exists library_item_versions_filters_idx on catalog.library_item_versions(content_format, level, visibility) where status = 'published';
create index if not exists library_item_versions_topics_idx on catalog.library_item_versions using gin(topics);
create index if not exists library_item_journey_links_journey_idx on catalog.library_item_journey_links(journey_version_id, library_item_version_id);
...
begin
  if not exists (select 1 from iam.permission_definitions where code = 'library.manage') then
    ...
    values (app_private.e14_deterministic_uuid('permission:library.manage'),'library.manage','library_content','manage','Create, edit and publish content library entries.');
  end if;
...
with event_names(event_name) as (
  values ('catalog.library_content.draft_saved'),('catalog.library_content.published'),('learning.library_content.accessed')
), documents as (
  select event_name,jsonb_build_object('type','object','title',event_name,'$schema','https://json-schema.org/draft/2020-12/schema','additionalProperties',true) as schema_document
  from event_names
)
...
select app_private.e14_deterministic_uuid('library:event-schema:' || event_name || ':1'),event_name,1,'urn:estimulo:event:' || event_name || ':1',schema_document,app_private.e14_request_hash(schema_document),'published',now()
from documents
