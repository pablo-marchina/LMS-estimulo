...
export async function JourneyProgressNav({ state, current, activityTitle, estimatedMinutes }: Props) {
  ...
  const title = current === "activity"
    ? activityTitle ?? "Atividade da jornada"
    : current === "result"
  ...
  let previousActivity: { activity_title: string } | null = null;
  let nextActivity: { activity_title: string } | null = null;
  ...
  const focusCopy = stepStatus === "completed"
    ? nextActivity
      ? `Aula concluída. A próxima é “${nextActivity.activity_title}”. Continue pela ação principal no fim da aula.`
      : previousActivity
        ? "Aula concluída. Você chegou ao fim das atividades desta jornada."
        : "Aula concluída. Continue pela ação principal no fim da aula."
      ...
      {current === "activity" && outline && journeyPercent !== null ? (
        <div className="mt-4 grid gap-3 border-t border-border pt-4 lg:grid-cols-[minmax(0,1fr)_minmax(16rem,0.48fr)]">
          <section className="rounded-xl bg-surface-muted/65 p-3.5" aria-label="Progresso geral da jornada">
            <div className="flex items-center justify-between gap-3 text-xs font-bold text-secondary"><span>Progresso da jornada</span><strong className="text-primary">{journeyPercent}%</strong></div>
            <Progress value={journeyPercent} tone="success" className="mt-2.5" />
            <p className="mt-2 text-xs leading-5 text-muted">{outline.completed_required_steps} de {outline.total_required_steps} atividades obrigatórias concluídas.</p>
          </section>
        ...
        <ol className="mt-4 grid gap-2 border-t border-border pt-4 sm:grid-cols-3" aria-label="Etapas da experiência">
          {stageOrder.map((stage, index) => {
            ...
            return <li key={stage} aria-current={active ? "step" : undefined} className={`flex items-center gap-3 rounded-xl border px-3 py-3 ${active ? "border-primary/30 bg-primary-soft" : completed ? "border-success/20 bg-success-soft/60" : "border-border bg-surface-muted/40"}`}>
              <span className={`grid size-8 shrink-0 place-items-center rounded-full text-xs font-black ${active ? "bg-primary text-white" : completed ? "bg-success text-white" : "bg-white text-muted ring-1 ring-border"}`}>{completed ? <CheckCircle2 size={17} aria-hidden="true" /> : index + 1}</span>
