...
export function AdminShell({ email, children }: { email: string; children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#fbfaf7] lg:grid" style={{ gridTemplateColumns: collapsed ? "68px minmax(0,1fr)" : "224px minmax(0,1fr)" }}>
    ...
    <aside className="no-print sticky top-0 hidden h-screen flex-col overflow-hidden border-r border-slate-200 bg-white text-ink lg:flex">
      <div className={`flex min-h-14 items-center border-b border-slate-200 ${collapsed ? "justify-center px-2" : "justify-between px-3"}`}><div className={collapsed ? "scale-75" : "scale-90 origin-left"}><EstimuloBrand href="/admin" compact /></div>{!collapsed ? <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[10px] font-semibold text-muted">Admin</span> : null}</div>
      <nav className={`grid min-h-0 flex-1 content-start gap-0.5 overflow-x-hidden ${settingsOpen ? "overflow-y-auto" : "overflow-y-hidden"} overscroll-contain p-2`} aria-label="Navegação administrativa">
        ...
        <div className="mt-1 border-t border-slate-200 pt-1.5">
          {settingsButton}
          {settingsOpen && !collapsed ? <div id="admin-settings-links" className="mt-1 grid gap-0.5 rounded-xl bg-slate-50 p-1.5">{settingsLinks.map((link) => renderLink(link, true))}</div> : null}
        </div>
      </nav>
      <div className="border-t border-slate-200 p-2">
        {!collapsed ? <p className="mb-1 truncate px-2 text-[10px] text-muted" title={email}>{email}</p> : null}
        <div className={`grid ${collapsed ? "gap-1" : "grid-cols-2 gap-1"}`}><form action={signOutAction}><Button variant="ghost" size="sm" type="submit" className="min-h-8 w-full gap-1.5 px-2 text-xs !text-muted hover:!bg-slate-100 hover:!text-ink"><LogOut size={16} aria-hidden="true" /><span className={collapsed ? "sr-only" : ""}>{signOutLabel}</span></Button></form><button type="button" onClick={() => setCollapsed((value) => !value)} className="flex min-h-8 items-center justify-center gap-1 rounded-lg px-2 text-[10px] font-semibold text-muted hover:bg-slate-100 hover:text-ink" aria-label={collapsed ? "Expandir menu" : "Recolher menu"}>{collapsed ? <ChevronRight size={18} /> : <><ChevronLeft size={16} /> Recolher</>}</button></div>
      </div>
    </aside>
    <div className="min-w-0">
      <header className="no-print sticky top-0 z-50 flex min-h-14 items-center border-b border-slate-200 bg-white px-4 text-ink lg:hidden"><div className="scale-90 origin-left"><EstimuloBrand href="/admin" compact /></div><button type="button" className="ml-auto grid size-9 place-items-center rounded-lg text-muted hover:bg-slate-100 hover:text-ink" aria-expanded={mobileOpen} aria-controls="admin-mobile-nav" aria-label={mobileOpen ? "Fechar menu" : "Abrir menu"} onClick={() => setMobileOpen((open) => !open)}>{mobileOpen ? <X size={20} /> : <Menu size={20} />}</button>{mobileOpen ? <div id="admin-mobile-nav" className="absolute inset-x-0 top-full max-h-[calc(100dvh-3.5rem)] overflow-y-auto border-t border-slate-200 bg-white p-3 shadow-xl"><nav className="grid gap-0.5">{links.map((link) => renderLink(link))}<div className="mt-1 border-t border-slate-200 pt-1.5">{settingsButton}{settingsOpen ? <div className="mt-1 grid gap-0.5 rounded-xl bg-slate-50 p-1.5">{settingsLinks.map((link) => renderLink(link, true))}</div> : null}</div></nav><div className="mt-2 border-t border-slate-200 pt-2"><p className="mb-1 truncate text-xs text-muted">{email}</p><form action={signOutAction}><Button variant="ghost" size="sm" type="submit" className="w-full !text-muted hover:!bg-slate-100 hover:!text-ink">{signOutLabel}</Button></form></div></div> : null}</header>
      <InterfaceSlot area="admin" placement="before_content" />
      <main id="conteudo-principal" className="mx-auto w-full max-w-[1440px] px-4 py-6 sm:px-6 sm:py-8 lg:px-8" tabIndex={-1}>
        {children}
