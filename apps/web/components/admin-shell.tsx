...
export function AdminShell({ email, children }: { email: string; children: React.ReactNode }) {
  ...
  const renderLink = (link: ShellLink, nested = false) => {
    ...
    const iconSize = collapsed ? 23 : nested ? 19 : 21;
    return <NavItem key={link.contentKey} href={link.href} variant="dark" exact={link.exact} icon={<Icon size={iconSize} strokeWidth={2.1} aria-hidden="true" />} interfaceContentKey={link.contentKey} className={`${collapsed ? "justify-center px-2" : ""} ${nested ? "min-h-10 rounded-lg pl-4 text-[12px]" : "min-h-11 text-[13px]"} gap-3 px-3 py-2 font-semibold`}><span className={collapsed ? "sr-only" : "truncate"}>{link.label}</span></NavItem>;
  };

  const settingsButton = <button type="button" onClick={() => { if (collapsed) setCollapsed(false); setSettingsOpen((open) => !open); }} className={`flex min-h-11 w-full items-center rounded-lg px-3 py-2 text-[13px] font-semibold text-white/85 transition hover:bg-white/10 hover:text-white ${collapsed ? "justify-center px-2" : "gap-3"}`} aria-expanded={settingsOpen} aria-controls="admin-settings-links" title={collapsed ? "Mais configurações" : undefined}><Settings size={collapsed ? 23 : 21} strokeWidth={2.1} aria-hidden="true" /><span className={collapsed ? "sr-only" : "flex-1 truncate text-left"}>Mais configurações</span>{!collapsed ? <ChevronDown size={18} className={`shrink-0 transition-transform ${settingsOpen ? "rotate-180" : ""}`} aria-hidden="true" /> : null}</button>;

  return <div className="min-h-screen bg-background lg:grid" style={{ gridTemplateColumns: collapsed ? "72px minmax(0,1fr)" : "236px minmax(0,1fr)" }}>
    ...
    <aside className="no-print sticky top-0 hidden h-screen flex-col overflow-hidden border-r border-primary-active bg-primary text-white shadow-sm lg:flex">
      <div className={`flex min-h-16 items-center border-b border-white/15 ${collapsed ? "justify-center px-2" : "justify-between px-3"}`}><div className={collapsed ? "scale-80" : "scale-95 origin-left"}><EstimuloBrand href="/admin" compact /></div>{!collapsed ? <span className="rounded-full border border-white/25 px-2.5 py-1 text-[11px] font-semibold text-white/85">Admin</span> : null}</div>
      <nav className="grid min-h-0 flex-1 content-start gap-1 overflow-y-auto overscroll-contain p-2.5" aria-label="Navegação administrativa">
        {links.map((link) => renderLink(link))}
        <div className="mt-1 border-t border-white/10 pt-2">
          {settingsButton}
          {settingsOpen && !collapsed ? <div id="admin-settings-links" className="mt-1 grid gap-1 rounded-xl bg-white/5 p-1.5">{settingsLinks.map((link) => renderLink(link, true))}</div> : null}
        </div>
      </nav>
      <div className="border-t border-white/15 p-2.5">
        {!collapsed ? <p className="mb-1.5 truncate px-2 text-[11px] text-white/65" title={email}>{email}</p> : null}
        <div className={`grid ${collapsed ? "gap-1" : "grid-cols-2 gap-1"}`}><form action={signOutAction}><Button variant="ghost" size="sm" type="submit" className="min-h-9 w-full gap-2 px-2 text-xs !text-white hover:!bg-white/10"><LogOut size={18} aria-hidden="true" /><span className={collapsed ? "sr-only" : ""}>{signOutLabel}</span></Button></form><button type="button" onClick={() => setCollapsed((value) => !value)} className="flex min-h-9 items-center justify-center gap-1.5 rounded-lg px-2 text-[11px] font-semibold text-white/70 hover:bg-white/10 hover:text-white" aria-label={collapsed ? "Expandir menu" : "Recolher menu"}>{collapsed ? <ChevronRight size={19} /> : <><ChevronLeft size={17} /> Recolher</>}</button></div>
      </div>
    </aside>
    <div className="min-w-0">
      <header className="no-print sticky top-0 z-50 flex min-h-16 items-center border-b border-primary-active bg-primary px-4 text-white shadow-sm lg:hidden"><div className="brand-logo-capsule scale-95 origin-left"><EstimuloBrand href="/admin" compact /></div><button type="button" className="ml-auto grid size-10 place-items-center rounded-lg hover:bg-white/10" aria-expanded={mobileOpen} aria-controls="admin-mobile-nav" aria-label={mobileOpen ? "Fechar menu" : "Abrir menu"} onClick={() => setMobileOpen((open) => !open)}>{mobileOpen ? <X size={22} /> : <Menu size={22} />}</button>{mobileOpen ? <div id="admin-mobile-nav" className="absolute inset-x-0 top-full max-h-[calc(100dvh-4rem)] overflow-y-auto border-t border-white/15 bg-primary p-3 shadow-xl"><nav className="grid gap-1">{links.map((link) => renderLink(link))}<div className="mt-1 border-t border-white/10 pt-2">{settingsButton}{settingsOpen ? <div className="mt-1 grid gap-1 rounded-xl bg-white/5 p-1.5">{settingsLinks.map((link) => renderLink(link, true))}</div> : null}</div></nav><div className="mt-2 border-t border-white/15 pt-2"><p className="mb-1 truncate text-xs text-white/65">{email}</p><form action={signOutAction}><Button variant="ghost" size="sm" type="submit" className="w-full !text-white hover:!bg-white/10">{signOutLabel}</Button></form></div></div> : null}</header>
