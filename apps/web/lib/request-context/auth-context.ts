...
async function resolveInterfacePreviewIdentity(
  identity: CurrentIdentityContext,
  ...
  if (
    identity.access_mode !== "administrative" ||
    !isEstimuloAdministrativeEmail(identity.authenticated_email)
  ) {
    throw new CurrentIdentityError(
      "INTERFACE_PREVIEW_ADMIN_REQUIRED",
      "Administrative access is required for participant preview.",
    );
  }

  const cookieStore = await cookies();
  const preview = parseInterfacePreviewIdentity(
    cookieStore.get(INTERFACE_PREVIEW_COOKIE)?.value,
  );

  if (!preview) {
    throw new CurrentIdentityError(
      "INTERFACE_PREVIEW_IDENTITY_INVALID",
      "The preview participant is invalid.",
    );
  }
  ...
  if (
    !organization ||
    organization.organization_id !== preview.organizationId
  ) {
    throw new CurrentIdentityError(
      "INTERFACE_PREVIEW_ORGANIZATION_INVALID",
      "The preview organization is invalid.",
    );
  }

  const workspace = await extensionsRuntime.adminWorkspace(
    identity.user_account_id,
    organization.organization_id,
  );
  const participant = workspace.participants.find(
    (item) =>
      item.user_account_id === preview.participantUserAccountId &&
      item.entrepreneur_id,
  );

  if (!participant?.entrepreneur_id) {
    throw new CurrentIdentityError(
      "INTERFACE_PREVIEW_PARTICIPANT_UNAVAILABLE",
      "No eligible participant is available for preview.",
    );
  }
...
export const getAuthContext = cache(async (): Promise<AuthContext> => {
  ...
  if (provider === "aws") {
    return {
      ...
      reason: "AWS_IDENTITY_ARCHITECTURE_PENDING",
    };
  }
  ...
  try {
    ...
    const identity =
      (await resolveInterfacePreviewIdentity(authenticatedIdentity)) ??
    ...
    const reason =
      error instanceof CurrentIdentityError
        ? error.code
