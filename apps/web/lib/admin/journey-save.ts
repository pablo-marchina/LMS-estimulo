...
function positiveInteger(value: string, fallback = 9999) {
...
function isUniqueConflict(error: unknown) {
  ...
  return /23505|duplicate key|unique constraint|already exists/i.test(message);
}
...
function configuration(formData: FormData) {
  ...
  try {
    ...
    throw new Error("INVALID_CONFIGURATION_SNAPSHOT");
  }
...
function recordText(value: unknown, field: string) {
  return value &&
    typeof value === "object" &&
    !Array.isArray(value) &&
...
function presentationTags(value: unknown) {
...
export function journeySaveErrorCode(error: unknown) {
  const message = error instanceof Error ? error.message : String(error);

  if (message.includes("ANNOUNCEMENT_FILE_SIZE_INVALID")) {
    return "imagem_tamanho_invalido";
  }
  if (message.includes("ANNOUNCEMENT_CONTENT_TYPE_NOT_ALLOWED")) {
    return "imagem_formato_invalido";
  }
  if (message.includes("ANNOUNCEMENT_FILE_EXTENSION_NOT_ALLOWED")) {
    return "imagem_extensao_invalida";
  }
  if (
    message.includes("ANNOUNCEMENT_STORAGE_UPLOAD_FAILED") ||
    message.includes("ANNOUNCEMENT_BUCKET_CREATE_FAILED")
  ) {
    return "imagem_upload_falhou";
  }
  if (message.includes("INVALID_CONFIGURATION_SNAPSHOT")) {
    return "configuracao_invalida";
  }
  if (message.includes("COMPLETION_CERTIFICATE_REQUIRED")) {
    return "certificado_conclusao_obrigatorio";
  }
  if (message.includes("COMPLETION_CERTIFICATE_NOT_AVAILABLE_FOR_JOURNEY")) {
    return "certificado_conclusao_incompativel";
  }
...
async function uploadJourneyCover(input: {
  ...
  try {
    ...
    const uploaded = await uploadLibraryContent({
      bucket,
      objectKey,
      file: input.file,
    });
    ...
    if (intentId) {
      await libraryRuntime
        .abortUpload(
          input.actor,
          input.organizationId,
          intentId,
          "JOURNEY_COVER_UPLOAD_FAILED",
          `${key}:abort`,
        )
        .catch(() => undefined);
    }

    if (objectCreated && objectKey) {
...
export async function saveAdminJourneyFromForm(input: {
  actorUserAccountId: string;
  organizationId: string;
  formData: FormData;
}) {
  const { actorUserAccountId, organizationId, formData } = input;
  ...
  const code =
    existingCode ||
    deriveCode(publicTitle, `jornada_${randomUUID().slice(0, 8)}`);
  const journeyId =
    nullable(formData, "journey_id") ?? nullable(formData, "version_id");
  const themeIds = [
    ...new Set(formData.getAll("theme_ids").map(String).filter(Boolean)),
  ];
  const completionCertificateEnabled = checked(
    formData,
    "completion_certificate_enabled",
  );
  const completionCertificateVersionId = nullable(
    formData,
    "completion_certificate_version_id",
  );
  ...
  if (completionCertificateEnabled) {
    if (!journeyId || !completionCertificateVersionId) {
      throw new Error("COMPLETION_CERTIFICATE_REQUIRED");
    }

    const productWorkspace = await getAdminProductWorkspace(
      actorUserAccountId,
      organizationId,
    ).catch(() => null);
    ...
    if (!compatibleCertificate) {
      throw new Error("COMPLETION_CERTIFICATE_NOT_AVAILABLE_FOR_JOURNEY");
    }
  }

  const previousPresentation =
    previousConfiguration.presentation &&
    typeof previousConfiguration.presentation === "object" &&
    !Array.isArray(previousConfiguration.presentation)
      ? (previousConfiguration.presentation as Record<string, unknown>)
      : {};

  const commandKey = randomUUID();
  let themeSaveFailed = false;
  let tags =
    themeIds.length === 0 ? [] : presentationTags(previousPresentation.tags);

  if (themeIds.length > 0) {
    try {
      const extensionWorkspace = await extensionsRuntime.adminWorkspace(
        actorUserAccountId,
        organizationId,
      );
      const themeNameById = new Map(
        extensionWorkspace.themes
          .filter((theme) => recordText(theme, "status") === "active")
          .map((theme) => [recordText(theme, "id"), recordText(theme, "name")]),
      );
      tags = themeIds
        .map((id) => themeNameById.get(id) ?? "")
        .filter(Boolean)
        .slice(0, 8);
    } catch {
  ...
  const cardBackgroundId = cardFile
    ? await uploadJourneyCover({
        actor: actorUserAccountId,
        organizationId,
        file: cardFile,
        role: "card",
      })
    : text(formData, "current_card_background_file_object_id") ||
      previousPresentation.card_background_file_object_id;
  const featuredBackgroundId = featuredFile
    ? await uploadJourneyCover({
        actor: actorUserAccountId,
        organizationId,
        file: featuredFile,
    ...
    : text(formData, "current_featured_background_file_object_id") ||
      previousPresentation.featured_background_file_object_id;
  ...
  const presentation = {
    ...
    ...(cardBackgroundId
      ? { card_background_file_object_id: String(cardBackgroundId) }
      : {}),
    ...(featuredBackgroundId
      ? { featured_background_file_object_id: String(featuredBackgroundId) }
      : {}),
    card_background_alt:
      text(formData, "card_background_alt") || `Capa da jornada ${publicTitle}`,
    featured_background_alt:
      text(formData, "featured_background_alt") ||
      `Imagem de destaque da jornada ${publicTitle}`,
  };

  const completionCertificate = {
    enabled: completionCertificateEnabled,
    certificate_version_id: completionCertificateEnabled
      ? completionCertificateVersionId
      : null,
  ...
  const payload = {
    ...
    eligible_archetype_codes: formData
      .getAll("eligible_archetype_codes")
      .map(String),
  };
  ...
  const savedJourneyId =
    (result as typeof result & { journey_id?: string }).journey_id ??
    result.version_id;
  ...
  try {
    await extensionsRuntime.saveAdmin({
      ...
      payload: {
        journey_definition_id: result.definition_id,
        theme_ids: themeIds,
      },
