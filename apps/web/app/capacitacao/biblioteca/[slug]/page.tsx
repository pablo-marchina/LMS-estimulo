      ...
      <article className="grid gap-8" aria-labelledby="library-content-title">
        ...
        {content.journeys.length ? (
          <Card>
            ...
            <ul className="mt-2 grid gap-1 text-sm text-muted">
              {content.journeys.map((journey) => <li key={journey.journey_version_id}>{journey.journey_title}</li>)}
            </ul>
          </Card>
        ) : null}
        ...
        {content.content_kind === "article" ? (
          <Card className="grid gap-3" id="conteudo" aria-labelledby="conteudo-titulo">
            <h2 id="conteudo-titulo" className="text-lg font-semibold text-ink">Conteúdo</h2>
          ...
          <Card className="grid gap-4" id="conteudo">
            <h2 className="text-lg font-semibold text-ink">Arquivo para baixar</h2>
            <p className="text-sm text-muted">O link é temporário e funciona apenas para participantes autorizados.</p>
            <p className="text-sm font-semibold text-ink">{content.original_filename ?? "Material da biblioteca"}</p>
            <ButtonLink href={`/api/library-content/${content.library_item_version_id}/download`} className="w-fit">
              Baixar arquivo
            </ButtonLink>
          </Card>
        ) : (
          <Card className="grid gap-4" id="conteudo">
            ...
            <p className="text-sm text-muted">O material será aberto no site da fonte. O acesso é registrado para medir uso da biblioteca, sem enviar o conteúdo da sua navegação ao parceiro.</p>
            <form action={openLibraryContentAction}>
              ...
              <Button type="submit" className="w-fit">Abrir material de {content.source_name}</Button>
            </form>
          </Card>
        )}
      </article>
