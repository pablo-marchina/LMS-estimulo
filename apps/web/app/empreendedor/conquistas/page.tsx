...
export default async function ParticipantAchievementsPage({ searchParams }: { searchParams: Promise<{ certificadoExterno?: string; codigo?: string; aviso?: string }> }) {
  ...
  return <div className="mx-auto grid max-w-[1400px] gap-9 px-5 py-8 lg:px-9 lg:py-10">
    ...
    <section className="grid gap-4" id="certificados-estimulo"><h2 className="display-font text-2xl text-secondary">Certificados de jornada</h2>{platformUnavailable ? <StatusPanel title="Certificados da plataforma temporariamente indisponíveis" tone="warning">Tente novamente em instantes.</StatusPanel> : credentials?.certificates.length === 0 ? <EmptyState icon={<ShieldCheck size={24} />} title="Nenhum certificado emitido" tone="info">Ao concluir uma jornada, o PDF aparecerá aqui automaticamente.</EmptyState> : <div className="grid gap-4 sm:grid-cols-2">{credentials?.certificates.map((certificate) => <Card key={certificate.issuance_id}><StatusPill tone={certificate.valid ? "success" : "warning"}>{certificate.valid ? "Válido" : "Indisponível"}</StatusPill><h3 className="mt-3 font-bold text-ink">{certificate.certificate_name}</h3><p className="mt-1 text-sm text-muted">{certificate.journey_title}</p><p className="mt-3 text-xs text-muted">Emitido em {dateFormatter.format(new Date(certificate.issued_at))}</p>{certificate.certificate_number ? <p className="mt-1 text-xs font-semibold text-muted">Número {certificate.certificate_number}</p> : null}<div className="mt-5 flex gap-2"><ButtonLink href={`/api/certificates/${certificate.issuance_id}/download`} size="sm" icon={<Download size={15} />}>Baixar PDF</ButtonLink><ButtonLink href={`/credenciais/${encodeURIComponent(certificate.verification_code)}`} variant="secondary" size="sm">Validar</ButtonLink></div></Card>)}</div>}</section>

    <section className="grid gap-5" id="certificados-externos"><div><h2 className="display-font text-2xl text-secondary">Certificados de outros cursos</h2><p className="mt-2 text-sm text-muted">{participantCopy.certificates.externalSectionDescription}</p></div>{query.certificadoExterno === "enviado" ? <StatusPanel title="Certificado adicionado" tone="success">{query.aviso === "atualizando" ? "O arquivo foi salvo. A carteira está terminando de atualizar e poderá aparecer após recarregar." : "O certificado está salvo na sua carteira."}</StatusPanel> : null}{query.certificadoExterno === "erro" ? <StatusPanel title="Não foi possível adicionar" tone="warning">{uploadErrors[query.codigo ?? ""] ?? "Revise os dados e tente novamente."}</StatusPanel> : null}
