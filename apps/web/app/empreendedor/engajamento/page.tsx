...
const submissionDateFormatter = new Intl.DateTimeFormat("pt-BR", {
  dateStyle: "medium",
  timeStyle: "short",
  timeZone: "America/Sao_Paulo"
});
...
export default async function ParticipantEngagementPage() {
  ...
  const [engagement, participantJourneys] = await Promise.all([
  ...
  return (
    <div className="mx-auto grid max-w-[1400px] gap-8 px-5 py-8 lg:px-9 lg:py-10">
      <PageHeader
        ...
        title="Engajamento"
        description="Acompanhe conquistas, pontos, posição no ranking e o retorno das suas entregas em um só lugar. Esses dados refletem aprendizagem e não afetam crédito."
      />
      ...
      <section className="grid gap-4" aria-labelledby="conquistas-titulo">
        ...
        {earned.length === 0 ? (
          <EmptyState title="Nenhuma conquista ainda" tone="info">
            Continue suas jornadas para desbloquear os primeiros selos e certificados.
          </EmptyState>
        ) : (
          ...
          <ol className="grid gap-2">
            {engagement.point_history.map((entry) => (
              <li key={entry.id} className="flex items-center gap-4 rounded-lg border border-border bg-surface px-4 py-3">
                <span className={`w-16 shrink-0 text-right text-sm font-bold tabular-nums ${entry.amount >= 0 ? "text-success" : "text-danger"}`}>
      ...
      <section className="grid gap-4" aria-labelledby="ranking-titulo">
        <div>
          <h2 id="ranking-titulo" className="display-font text-2xl text-ink">Ranking</h2>
          <p className="mt-1 text-sm text-muted">Sua posição considera somente pontos de aprendizagem. Outros participantes aparecem por pseudônimo.</p>
        </div>
        {engagement.ranking.length === 0 ? (
          ...
          <ol className="grid gap-2">
            {engagement.ranking.map((entry) => (
              <li key={`${entry.position}:${entry.participant}`} className={`flex items-center gap-4 rounded-lg border px-4 py-3 ${entry.is_current ? "border-border-strong bg-primary-soft" : "border-border bg-surface"}`}>
      ...
      <section className="grid gap-4" aria-labelledby="entregas-titulo">
        <div>
          <h2 id="entregas-titulo" className="display-font text-2xl text-ink">Entregas</h2>
          <p className="mt-1 text-sm text-muted">Acompanhe arquivos enviados e o status de revisão de cada atividade prática.</p>
        </div>
        {groupsWithSubmissions.length === 0 ? (
          <EmptyState title="Nenhuma entrega enviada ainda" tone="info">
            Quando você enviar uma evidência em uma atividade prática, ela aparecerá aqui.
          </EmptyState>
        ) : (
          groupsWithSubmissions.map(({ journey, result }) => (
            <div key={journey.journey_instance_id} className="grid gap-3">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {result!.submissions.map((submission) => (
                  <Card key={submission.id} className="flex flex-col">
                    ...
                    <h4 className="truncate font-semibold text-ink" title={submission.original_filename ?? undefined}>
                      {submission.original_filename ?? "Arquivo enviado"}
                    </h4>
                    <p className="mt-1 text-xs text-muted">{submissionDateFormatter.format(new Date(submission.submitted_at))}</p>
      ...
      <div className="no-print flex flex-wrap items-center justify-between gap-3 border-t border-border pt-6">
        ...
        <ButtonLink href="/empreendedor/credenciais" variant="secondary">Ver carteira de credenciais</ButtonLink>
      </div>
    </div>
  );
}
