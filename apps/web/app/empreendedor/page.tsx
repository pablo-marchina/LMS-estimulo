...
export default async function ParticipantHome() {
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-5 py-6 lg:px-9 lg:py-8">
      ...
      {diagnosticPending ? <Card className="mt-6 border-primary/25 bg-primary-soft/45"><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-bold text-primary">Uma forma de personalizar</p><h1 className="mt-1 text-xl font-black text-secondary">Conheça seu perfil empreendedor</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">O diagnóstico tem 12 perguntas e ajuda a organizar recomendações. Ele é opcional e não impede o acesso às jornadas abertas.</p></div><div className="flex shrink-0 flex-wrap gap-2"><form action={startProfileDiagnosticAction}><Button type="submit">Fazer diagnóstico</Button></form><ButtonLink href="/empreendedor/jornadas" variant="secondary">Explorar jornadas</ButtonLink></div></div></Card> : null}

      <header className="brand-welcome-strip mt-8"><div><p className="inline-flex items-center gap-2 text-sm font-bold text-primary"><Sparkles size={16} /> Olá, {firstName}!</p><h1 className="display-font mt-2 text-4xl text-secondary sm:text-5xl">Vamos mover seu negócio?</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-muted sm:text-base">Escolha uma aula, aplique no seu ritmo e transforme aprendizado em decisões concretas.</p></div><div className="brand-welcome-bubbles" aria-hidden="true"><span /><span /><span /></div></header>
