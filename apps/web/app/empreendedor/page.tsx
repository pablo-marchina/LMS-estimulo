...
import { BookOpen, Play, Rocket, Sparkles, Trophy } from "lucide-react";
...
import { ParticipantHomeBadgeHighlights } from "@/components/participant-home-badge-highlights";
...
export default async function ParticipantHome() {
  ...
  const eligibleJourneys = fulfilled(results[2]) ?? [];
  const diagnosticEntry = fulfilled(results[3]);
  ...
  const availableJourneys = eligibleJourneys
    ...
    .sort((a, b) => Number(Boolean(b.presentation?.featured)) - Number(Boolean(a.presentation?.featured)) || rank(a) - rank(b))
    .slice(0, 3);
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-5 py-6 lg:px-9 lg:py-8">
      ...
      <ParticipantHomeBadgeHighlights actorUserAccountId={auth.identity.user_account_id} />
    </div>
  );
}
