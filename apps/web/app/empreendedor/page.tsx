...
import { Bot, BookOpen, Play, Trophy } from "lucide-react";
...
export default async function ParticipantHome() {
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-5 py-8 lg:px-9 lg:py-10">
      <header><p className="text-sm font-semibold text-muted">Olá, {firstName} 👋</p><h1 className="display-font mt-2 text-4xl text-secondary sm:text-5xl">Vamos mover seu negócio?</h1></header>
      ...
      {!nextJourney ? (
        ...
        <section className="mt-8 grid gap-5 xl:grid-cols-[1.35fr_.65fr]" aria-label="Continue de onde parou">
          <article className="brand-hero animate-in relative overflow-hidden rounded-card p-7 text-white sm:p-9" aria-labelledby="proxima-acao-titulo">
            <div className="relative z-10 max-w-2xl">
              ...
              <p className="mt-5 max-w-xl leading-7 text-white/70">{nextJourney.journey_description ?? participantCurrentStageLabel(nextJourney)}</p>
              <div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-white/85"><StatusPill tone="expressive" className="!bg-white/15 !text-white">{statusLabel(nextJourney.journey_status)}</StatusPill><span>{nextJourney.completed_required_steps} de {nextJourney.total_required_steps} etapas obrigatórias concluídas</span></div>
              <div className="mt-8">
          ...
          <article className="rounded-card border border-border bg-brand-green p-6"><Trophy aria-hidden="true" /><p className="display-font mt-6 text-5xl text-secondary">{totalPoints}</p><h2 className="mt-3 text-xl font-black text-secondary">Pontos de evolução</h2><p className="mt-2 text-sm text-secondary/65">Seus pontos refletem atividades realizadas na plataforma.</p></article>
        </section>
      )}
      ...
      <section className="mt-8 grid gap-4" aria-labelledby="jornadas-disponiveis-titulo">
        <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm font-semibold text-muted">Próximos caminhos</p><h2 id="jornadas-disponiveis-titulo" className="display-font mt-1 text-2xl text-secondary">Jornadas disponíveis para você</h2></div><Link href="/empreendedor/jornadas" className="text-sm font-semibold text-primary hover:underline">Ver todas as jornadas disponíveis</Link></div>
        {availableJourneys.length ? <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">{availableJourneys.map((journey) => {
          const isOpenAI = journey.journey_version_id === OPENAI_JOURNEY_VERSION_ID;
          return <Card key={journey.journey_version_id} className={isOpenAI ? "brand-accent-card border-primary/30 bg-primary-soft/35" : "flex flex-col"}>{isOpenAI ? <span className="mb-3 grid size-10 place-items-center rounded-xl bg-primary text-white"><Bot size={20} /></span> : null}<StatusPill tone={isOpenAI ? "expressive" : journey.open_to_all ? "neutral" : "info"}>{isOpenAI ? "Jornada OpenAI em destaque" : journey.open_to_all ? "Aberta para todos" : "Indicada para seu perfil"}</StatusPill><h3 className="mt-3 font-bold text-secondary">{journey.title}</h3>{journey.description ? <p className="mt-2 text-sm text-muted">{journey.description}</p> : null}<Link href="/empreendedor/jornadas" className="mt-auto pt-4 text-sm font-semibold text-primary hover:underline">Ver detalhes e entrar</Link></Card>;
        })}</div> : <p className="rounded-card border border-border bg-white p-5 text-sm text-muted">Você já entrou em todas as jornadas disponíveis no momento.</p>}
      </section>
      ...
      <section className="mt-6 flex flex-wrap items-center justify-between gap-4 rounded-card border border-border bg-white p-5" aria-labelledby="recompensas-resumo-titulo">
        ...
        <Link href="/empreendedor/conquistas" className="text-sm font-semibold text-primary hover:underline">Ver selos e certificados</Link>
      </section>
    </div>
  );
}
