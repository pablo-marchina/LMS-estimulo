...
import { ClipboardList, Compass, FileUp, Info, Mail, Sparkles, Star, Target, Trophy, UserRound } from "lucide-react";
...
type ProfileTab = "informacoes" | "diagnostico" | "entregas";
...
function resolveTab(value: string | undefined): ProfileTab { return value === "diagnostico" || value === "entregas" ? value : "informacoes"; }

export default async function ParticipantProfilePage({ searchParams }: { searchParams: Promise<{ aba?: string; erro?: string; sucesso?: string; referencia?: string; diagnostico?: string }> }) {
  const query = await searchParams;
  const activeTab = resolveTab(query.aba);
  ...
  return <div className="mx-auto grid max-w-[1400px] gap-8 px-5 py-8 lg:px-9 lg:py-10">
    <PageHeader eyebrow="Minha conta" title="Perfil" description="Consulte seus dados, diagnósticos e materiais enviados em um só lugar." />

    <nav aria-label="Seções do perfil" className="flex flex-wrap gap-2 rounded-2xl border border-border bg-white p-2 shadow-sm">
      <ProfileTabLink href="/empreendedor/perfil?aba=informacoes" active={activeTab === "informacoes"} icon={<Info size={17} />}>Informações</ProfileTabLink>
      <ProfileTabLink href="/empreendedor/perfil?aba=diagnostico" active={activeTab === "diagnostico"} icon={<ClipboardList size={17} />}>Diagnóstico</ProfileTabLink>
      <ProfileTabLink href="/empreendedor/perfil?aba=entregas" active={activeTab === "entregas"} icon={<FileUp size={17} />}>Entregas</ProfileTabLink>
    </nav>
    ...
    {activeTab === "entregas" ? <>
      {query.sucesso ? <StatusPanel title="Material enviado" tone="success">A entrega foi registrada e já aparece no histórico.</StatusPanel> : null}
      {query.erro ? <StatusPanel title="Não foi possível enviar o material" tone="warning">Revise o arquivo e tente novamente.</StatusPanel> : null}
      <ParticipantProfileMaterials embedded />
    </> : null}
  </div>;
}

function ProfileTabLink({ href, active, icon, children }: { href: string; active: boolean; icon: React.ReactNode; children: React.ReactNode }) {
  return <Link href={href} aria-current={active ? "page" : undefined} className={`inline-flex min-h-11 items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold transition ${active ? "bg-primary text-white shadow-sm" : "text-secondary hover:bg-primary-soft hover:text-primary"}`}>{icon}{children}</Link>;
}
