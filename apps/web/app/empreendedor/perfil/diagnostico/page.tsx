...
export default async function ParticipantProfileDiagnosticPage({ searchParams }: { searchParams: Promise<{ erro?: string; diagnostico?: string; sucesso?: string; referencia?: string }> }) {
  ...
  const optionalDiagnostics = extensions?.optional_diagnostics ?? [];
  ...
  return <div className="grid gap-8">
    ...
    {optionalDiagnostics.length ? <section aria-labelledby="diagnosticos-opcionais-titulo"><div className="mb-4"><p className="brand-kicker">Autoconhecimento</p><h2 id="diagnosticos-opcionais-titulo" className="display-font mt-1 text-2xl text-secondary">Diagnósticos opcionais</h2><p className="mt-1 text-sm text-muted">Você escolhe se quer fazer. Eles não alteram seu arquétipo nem o acesso às jornadas.</p></div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{optionalDiagnostics.map((diagnostic) => <OptionalDiagnosticCard key={text((diagnostic.availability as JsonRecord | undefined)?.id)} diagnostic={diagnostic} />)}</div></section> : null}
  </div>;
}
