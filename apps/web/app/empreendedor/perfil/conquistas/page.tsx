...
const uploadErrors: Record<string, string> = {
  ...
  EXTERNAL_CREDENTIAL_PROJECTION_INCONSISTENT:
    "O certificado foi salvo, mas a carteira ainda não foi atualizada. Recarregue a página.",
};
...
export default async function ParticipantProfileAchievementsPage({
  searchParams,
}: {
  searchParams: Promise<{ certificadoExterno?: string; codigo?: string; aviso?: string }>;
}) {
  ...
  return (
    <div className="grid gap-9 pb-8">
      <div>
        <p className="text-sm font-semibold uppercase tracking-[0.18em] text-primary">Seu reconhecimento</p>
        <h1 className="mt-2 display-font text-3xl text-secondary">Conquistas e certificados</h1>
        <p className="mt-2 max-w-3xl text-sm text-muted">{participantCopy.certificates.pageDescription}</p>
      </div>

      {platformUnavailable || externalUnavailable ? (
        <StatusPanel title="A carteira não pôde ser atualizada por completo" tone="warning">
          Nenhum certificado foi apagado. Recarregue a página para tentar novamente.
        </StatusPanel>
      ) : null}
      ...
      <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <MetricTile
          index={0}
          label="Selos"
          value={platformUnavailable ? "—" : credentials?.badges.filter((item) => item.status === "active").length ?? 0}
        />
        <MetricTile index={1} label="Certificados Estímulo" value={platformUnavailable ? "—" : credentials?.certificates.length ?? 0} />
        <MetricTile
          index={2}
          label="Certificados externos"
          value={externalUnavailable ? "—" : external?.count ?? 0}
          meta={externalUnavailable ? "Não foi possível atualizar" : undefined}
        />
        <MetricTile index={3} label="Próximas conquistas" value={results[0].status === "rejected" ? "—" : available.length} />
      </section>

      <section className="grid gap-4">
        <h2 className="display-font text-2xl text-secondary">Selos conquistados</h2>
        {platformUnavailable ? (
          <StatusPanel title="Selos temporariamente indisponíveis" tone="warning">Tente novamente em instantes.</StatusPanel>
        ) : credentials?.badges.length === 0 ? (
          <EmptyState icon={<Sparkles size={24} />} title="Seu primeiro selo está a caminho" tone="info">
            Conclua jornadas para desbloquear reconhecimentos.
          </EmptyState>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {credentials?.badges.map((badge) => (
              <Card key={badge.award_id} className="border-success/30 bg-success-soft/35">
                <StatusPill tone={badge.status === "active" ? "success" : "neutral"}>{badge.status === "active" ? "Conquistado" : "Revogado"}</StatusPill>
                <div className="mt-5 flex gap-4">
                  <span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-brand-green text-secondary"><Award /></span>
                  <div>
                    <h3 className="font-bold text-ink">{badge.title}</h3>
                    <p className="mt-1 text-sm text-muted">{badge.description}</p>
                    <p className="mt-3 text-xs font-semibold text-primary">{badge.journey_title}</p>
                  </div>
                </div>
              </Card>
            ))}
      ...
      <section className="grid gap-4" id="certificados-estimulo">
        <h2 className="display-font text-2xl text-secondary">Certificados de jornada</h2>
        {platformUnavailable ? (
          <StatusPanel title="Certificados da plataforma temporariamente indisponíveis" tone="warning">Tente novamente em instantes.</StatusPanel>
        ) : credentials?.certificates.length === 0 ? (
          <EmptyState icon={<ShieldCheck size={24} />} title="Nenhum certificado emitido" tone="info">
            Ao concluir uma jornada, o PDF aparecerá aqui automaticamente.
          </EmptyState>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2">
            {credentials?.certificates.map((certificate) => (
              <Card key={certificate.issuance_id}>
                <StatusPill tone={certificate.valid ? "success" : "warning"}>{certificate.valid ? "Válido" : "Indisponível"}</StatusPill>
                <h3 className="mt-3 font-bold text-ink">{certificate.certificate_name}</h3>
                <p className="mt-1 text-sm text-muted">{certificate.journey_title}</p>
                <p className="mt-3 text-xs text-muted">Emitido em {dateFormatter.format(new Date(certificate.issued_at))}</p>
                {certificate.certificate_number ? <p className="mt-1 text-xs font-semibold text-muted">Número {certificate.certificate_number}</p> : null}
                <div className="mt-5 flex gap-2">
                  <ButtonLink href={`/api/certificates/${certificate.issuance_id}/download`} size="sm" icon={<Download size={15} />}>Baixar PDF</ButtonLink>
                  <ButtonLink href={`/credenciais/${encodeURIComponent(certificate.verification_code)}`} variant="secondary" size="sm">Validar</ButtonLink>
                </div>
              </Card>
            ))}
          </div>
      ...
      <section className="grid gap-5" id="certificados-externos">
        ...
        {query.certificadoExterno === "enviado" ? (
          <StatusPanel title="Certificado adicionado" tone="success">
            {query.aviso === "atualizando" ? "O arquivo foi salvo. A carteira está terminando de atualizar e poderá aparecer após recarregar." : "O certificado está salvo na sua carteira."}
          </StatusPanel>
        ) : null}
        {query.certificadoExterno === "erro" ? (
          <StatusPanel title="Não foi possível adicionar" tone="warning">{uploadErrors[query.codigo ?? ""] ?? "Revise os dados e tente novamente."}</StatusPanel>
        ) : null}
        ...
        {externalUnavailable ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {external?.items.map((item) => (
              <Card key={item.id}>
                <StatusPill tone={item.storage_status === "ready" ? "info" : "warning"}>{item.storage_status === "ready" ? "Externo" : "Arquivo em verificação"}</StatusPill>
                <h3 className="mt-3 font-bold text-ink">{item.title}</h3>
                <p className="mt-1 text-sm text-muted">{item.issuer}</p>
                <div className="mt-4 flex gap-2">
                  {item.download_available ? <ButtonLink href={`/api/external-credentials/${item.id}/download`} variant="secondary" size="sm" icon={<Download size={14} />}>Baixar</ButtonLink> : null}
                  {item.verification_url ? <ButtonLink href={item.verification_url} variant="ghost" size="sm">Validar</ButtonLink> : null}
                </div>
              </Card>
            ))}
          </div>
      ...
      <section className="grid gap-4">
        <h2 className="display-font text-2xl text-secondary">O que você ainda pode conquistar</h2>
        {results[0].status === "rejected" ? (
          <StatusPanel title="Reconhecimentos temporariamente indisponíveis" tone="warning">Tente novamente em instantes.</StatusPanel>
        ) : available.length === 0 ? (
          <EmptyState title="Nenhuma recompensa pendente" tone="success">Você já conquistou tudo disponível.</EmptyState>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {available.map((reward) => (
              <Card key={`${reward.type}:${reward.version_id}`}>
                <StatusPill tone="neutral">Disponível</StatusPill>
                <h3 className="mt-3 font-semibold text-ink">{reward.title}</h3>
