...
function jsonErrorCode(field: string) {
  const suffix = field.toUpperCase().replace(/[^A-Z0-9]+/gu, "_").replace(/^_+|_+$/gu, "").slice(0, 48);
  return suffix ? `INVALID_JSON_${suffix}` : "INVALID_JSON";
}

function parseJson(raw: string, fallback: unknown, field: string) {
  if (!raw) return fallback;
  try {
    return JSON.parse(raw) as unknown;
  } catch {
    throw new Error(jsonErrorCode(field));
  }
}
...
function payloadFromForm(formData: FormData): JsonRecord {
  ...
  for (const key of fieldNames) {
    ...
    if (jsonFields.has(key)) {
      payload[key] = parseJson(value, value.trim().startsWith("[") ? [] : {}, key);
      continue;
    }
