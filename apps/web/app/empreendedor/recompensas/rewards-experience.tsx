...
function RewardCard({ reward, balance }: { reward: JsonRecord; balance: number }) {
  ...
  return <Card className={`flex flex-col overflow-hidden p-0 transition hover:-translate-y-1 hover:shadow-xl ${unlocked ? "ring-1 ring-primary/15" : ""}`}>
    ...
    <div className="flex flex-1 flex-col p-5"><div className="flex items-start justify-between gap-3"><span className={`grid size-12 shrink-0 place-items-center rounded-2xl ${unlocked ? "bg-primary text-white shadow-lg" : "bg-surface-muted text-muted"}`}>{unlocked ? <Gift size={24} /> : <LockKeyhole size={22} />}</span></div><h3 className="mt-4 text-xl font-black text-ink">{text(reward.name)}</h3><p className="mt-2 flex-1 text-sm leading-6 text-muted">{text(reward.description)}</p><div className="mt-5 flex items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-muted">Custo</p><p className="display-font text-3xl text-secondary">{cost}</p></div><span className={`text-xs font-bold ${unlocked ? "text-success" : "text-muted"}`}>{unlocked ? "Pronta para resgatar" : `Faltam ${missing}`}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-surface-muted"><div className="h-full rounded-full bg-primary" style={{ width: `${progress}%` }} /></div><details className="mt-4 rounded-xl border border-border"><summary className="cursor-pointer p-3 text-sm font-semibold text-secondary">Ver detalhes</summary><div className="border-t border-border p-3 text-sm whitespace-pre-wrap text-muted">{text(reward.regulation)}</div></details><form action={performExtensionAction} className="mt-4 grid gap-3"><input type="hidden" name="action_type" value="reward_redeem" /><input type="hidden" name="return_to" value="/empreendedor/recompensas" /><input type="hidden" name="reward_id" value={text(reward.id)} /><input type="hidden" name="json_fields" value="fulfillment_details" /><input type="hidden" name="fulfillment_details" value="{}" /><PendingSubmitButton pendingLabel="Solicitando…" disabled={!unlocked}>{unlocked ? "Resgatar recompensa" : `Junte mais ${missing} pontos`}</PendingSubmitButton></form></div>
  </Card>;
}
