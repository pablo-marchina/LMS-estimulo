...
export async function signUpAction(formData: FormData) {
  ...
  if (!preferredName || !email || !password || !passwordConfirmation) redirect("/cadastro/teste?erro=campos_obrigatorios");
  if (!validEmail(email)) redirect("/cadastro/teste?erro=email_invalido");
  if (password.length < 10) redirect("/cadastro/teste?erro=senha_curta");
  if (password !== passwordConfirmation) redirect("/cadastro/teste?erro=senhas_diferentes");
  ...
  if (createError || !created.user) {
    ...
    if (code.includes("email") || message.includes("already") || message.includes("registered")) {
      redirect("/cadastro/teste?erro=usuario_existente");
    }
    redirect("/cadastro/teste?erro=criacao_falhou");
  }

  const session = await createSessionClient();
  const { error: signInError } = await session.auth.signInWithPassword({ email, password });
  if (signInError) redirect("/cadastro/teste?erro=autenticacao_falhou");

  const auth = await getAuthContext();
  if (auth.status !== "authenticated") redirect("/cadastro/teste?erro=provisionamento_falhou");
  ...
  try {
    ...
    redirect("/cadastro/teste?erro=provisionamento_falhou");
  }
