...
export function DiagnosticBuilder({ initial, previousProfiles, canPublish }: DiagnosticBuilderProps) {
  ...
  function updateProfileCode(index: number, nextCode: string) {
    ...
    updateProfile(index, { code: codeFrom(nextCode, `perfil_${index + 1}`) });
    if (defaultProfileCode === previousCode) setDefaultProfileCode(codeFrom(nextCode, `perfil_${index + 1}`));
    setMapping((current) => Object.fromEntries(Object.entries(current).map(([oldCode, targetCode]) => [oldCode, targetCode === previousCode ? codeFrom(nextCode, `perfil_${index + 1}`) : targetCode])));
  }
  ...
  return <form action={saveDiagnosticAction} className="grid gap-4">
    ...
    <AdminDisclosure title="Dimensões avaliadas" description="Crie os eixos necessários para organizar as perguntas e a classificação.">
      <div className="grid gap-3">{dimensions.map((dimension, index) => <div key={`dimension-${index}`} className="grid gap-3 rounded-xl border border-border bg-white p-4 sm:grid-cols-2"><Label>Nome da dimensão<Input name={`dimension_name_${index}`} value={dimension.name} onChange={(event) => updateDimension(index, { name: event.target.value })} required /></Label><Label>Código interno<Input name={`dimension_code_${index}`} value={dimension.code} onChange={(event) => updateDimensionCode(index, event.target.value)} pattern="[a-z][a-z0-9_-]{1,79}" required /></Label><Label className="sm:col-span-2">Descrição<Textarea name={`dimension_description_${index}`} rows={2} value={dimension.description} onChange={(event) => updateDimension(index, { description: event.target.value })} /></Label><Button type="button" variant="secondary" size="sm" className="w-fit" icon={<Trash2 size={14} />} disabled={dimensions.length <= 1} onClick={() => removeDimension(index)}>Remover dimensão</Button></div>)}</div>
