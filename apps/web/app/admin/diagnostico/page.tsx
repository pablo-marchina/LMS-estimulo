...
export default async function AdminDiagnosticPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <AppShell area="admin" email={auth.email}><div className="grid gap-6">
    ...
    {type === "principal" ? <>
      <StatusPanel title="O que o diagnóstico principal controla" tone="info">Ele define o arquétipo principal e pode determinar quais jornadas ficam disponíveis. Ao publicar uma mudança, a plataforma pede a relação entre os perfis antigos e os novos.</StatusPanel>
