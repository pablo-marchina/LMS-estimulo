...
import { publishAdminInterfaceContent, saveAdminInterfaceContent } from "@/lib/interface-content/runtime";
...
export async function saveInterfaceContentAction(formData: FormData) {
  ...
  const count = Math.max(0, Number.parseInt(text(formData, "entry_count"), 10) || 0);
