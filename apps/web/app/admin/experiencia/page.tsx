...
export default async function AdminExperiencePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  try {
    ...
    return <AppShell area="admin" email={auth.email}><StatusPanel title="CMS temporariamente indisponível" tone="warning">Não foi possível carregar a configuração da interface. Nenhuma alteração foi aplicada. Tente novamente mais tarde.</StatusPanel></AppShell>;
  }
