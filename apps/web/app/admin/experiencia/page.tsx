...
export default async function AdminExperiencePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const workspace = await getAdminInterfaceContent({ actorUserAccountId: auth.identity.user_account_id, organizationId: organization.organization_id }).catch(() => ({ organization_id: organization.organization_id, locale: "pt-BR", entries: [] as AdminInterfaceContentEntry[] }));
