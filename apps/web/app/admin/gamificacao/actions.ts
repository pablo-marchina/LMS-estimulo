...
function recurrencePolicy(formData: FormData): Record<string, unknown> {
  ...
  const scope: Record<string, string> = { once: "participant", per_activity: "enrollment_activity", per_assessment: "enrollment_assessment", daily: "participant_day", weekly: "participant_week", unlimited: "event" };
  return { scope: scope[frequency] ?? "participant", ...(frequency === "unlimited" ? {} : { maximum }), transferable: false };
}
function validityPolicy(formData: FormData): Record<string, unknown> {
  const mode = text(formData, "validity_mode") || "never";
  return mode === "months" ? { expires: true, duration_months: positiveInteger(text(formData, "validity_months"), 12) } : { expires: false };
}
...
export async function saveGamificationResourceAction(formData: FormData) {
  ...
  if (resourceType === "point_rule") {
    ...
    requestedCertificateStatus = text(formData, "status") || "draft";
  ...
  try {
    ...
    if (resourceType === "certificate") {
      ...
      await extendedCredentialRuntime.configureCertificate({
        ...
        templateLayout: {
          name_y: boundedRatio(text(formData, "name_y_percent"), .53),
          journey_y: boundedRatio(text(formData, "journey_y_percent"), .40),
          text_color: ["primary", "white"].includes(text(formData, "text_color")) ? text(formData, "text_color") : "primary",
        },
