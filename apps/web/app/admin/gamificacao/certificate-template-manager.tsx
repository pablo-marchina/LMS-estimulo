...
export async function CertificateTemplateManager() {
  ...
  return <section id="templates-certificado" className="grid scroll-mt-24 gap-4">
    ...
    <Card className="grid gap-3">
      <div><h3 className="font-black text-secondary">Modelos em uso</h3><p className="text-sm text-muted">A plataforma escolhe automaticamente nesta ordem: jornada, programa e modelo geral. Clique na prévia para abrir o arquivo original.</p></div>
    ...
    <details className="rounded-2xl border border-border bg-white shadow-sm" open={assignments.length === 0}><summary className="cursor-pointer p-4 font-bold text-secondary">Arquivos já enviados</summary><div className="grid gap-3 border-t border-border p-4 sm:grid-cols-2 xl:grid-cols-3">{assets.map((asset) => {
      const assetId = text(asset.id);
      const src = previewUrl(organization.organization_id, asset);
      return <article key={assetId} className="overflow-hidden rounded-xl border border-border bg-surface-muted"><PersistedTemplatePreview organizationId={organization.organization_id} asset={asset} className="aspect-[1.414/1] w-full rounded-none border-0" /><div className="grid gap-3 p-3"><div><strong className="block truncate text-ink">{assetName(asset)}</strong><span className="text-xs text-muted">{text(asset.media_type) === "pdf" ? "PDF" : "Imagem"} · {Math.max(1, Math.round(Number(asset.size_bytes ?? 0) / 1024))} KB</span>{src ? <a href={src} target="_blank" rel="noreferrer" className="mt-1 flex w-fit items-center gap-1 text-xs font-bold text-primary hover:underline">Visualizar <ExternalLink size={12} /></a> : null}</div><ExistingAssetAssignment assetId={assetId} programs={programs} journeys={journeys} /></div></article>;
...
function ExistingAssetAssignment({ assetId, programs, journeys }: { assetId: string; programs: { id: string; name: string }[]; journeys: { id: string; name: string }[] }) {
  return <details className="rounded-lg border border-border bg-white"><summary className="cursor-pointer px-3 py-2 text-sm font-bold text-primary">Usar este arquivo</summary><div className="grid gap-3 border-t border-border p-3"><form action={saveExtensionAction}><input type="hidden" name="resource_type" value="certificate_template_assignment" /><input type="hidden" name="return_to" value={returnTo} /><input type="hidden" name="scope_type" value="global" /><input type="hidden" name="scope_id" value="" /><input type="hidden" name="template_asset_id" value={assetId} /><Button type="submit" variant="secondary" size="sm" className="w-full">Usar como modelo geral</Button></form>{programs.length ? <form action={saveExtensionAction} className="grid gap-2"><input type="hidden" name="resource_type" value="certificate_template_assignment" /><input type="hidden" name="return_to" value={returnTo} /><input type="hidden" name="scope_type" value="program" /><input type="hidden" name="template_asset_id" value={assetId} /><Label className="text-xs">Programa<Select name="scope_id" required><option value="">Selecione</option>{programs.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</Select></Label><Button type="submit" variant="secondary" size="sm">Usar no programa</Button></form> : null}{journeys.length ? <form action={saveExtensionAction} className="grid gap-2"><input type="hidden" name="resource_type" value="certificate_template_assignment" /><input type="hidden" name="return_to" value={returnTo} /><input type="hidden" name="scope_type" value="journey" /><input type="hidden" name="template_asset_id" value={assetId} /><Label className="text-xs">Jornada<Select name="scope_id" required><option value="">Selecione</option>{journeys.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</Select></Label><Button type="submit" variant="secondary" size="sm">Usar na jornada</Button></form> : null}</div></details>;
}
