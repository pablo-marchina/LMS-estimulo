...
function previewUrl(organizationId: string, asset: JsonRecord | undefined) {
  ...
  if (!fileObjectId || text(asset?.media_type) === "pdf") return "";
...
function PersistedTemplatePreview({ organizationId, asset, className = "" }: { organizationId: string; asset: JsonRecord | undefined; className?: string }) {
  ...
  if (!src) return <span className={`grid place-items-center rounded-lg bg-primary-soft text-primary ${className}`} aria-hidden="true">{text(asset?.media_type) === "pdf" ? <FileText size={20} /> : <FileImage size={20} />}</span>;
  return <img src={src} alt={`Prévia de ${assetName(asset)}`} loading="lazy" className={`rounded-lg border border-border bg-white object-cover ${className}`} />;
}
...
export async function CertificateTemplateManager() {
  ...
  return <section id="templates-certificado" className="grid scroll-mt-24 gap-4">
    ...
    <Card className="grid gap-3">
      <div><h3 className="font-black text-secondary">Modelos em uso</h3><p className="text-sm text-muted">A plataforma escolhe automaticamente nesta ordem: jornada, programa e modelo geral. A prévia abaixo permanece disponível depois do envio.</p></div>
    ...
    <details className="rounded-2xl border border-border bg-white shadow-sm"><summary className="cursor-pointer p-4 font-bold text-secondary">Arquivos já enviados</summary><div className="grid gap-3 border-t border-border p-4 sm:grid-cols-2 xl:grid-cols-3">{assets.map((asset) => <article key={text(asset.id)} className="overflow-hidden rounded-xl border border-border bg-surface-muted"><PersistedTemplatePreview organizationId={organization.organization_id} asset={asset} className="aspect-[1.414/1] w-full rounded-none border-0" /><div className="p-3"><strong className="block truncate text-ink">{assetName(asset)}</strong><span className="text-xs text-muted">{text(asset.media_type) === "pdf" ? "PDF" : "Imagem"} · {Math.max(1, Math.round(Number(asset.size_bytes ?? 0) / 1024))} KB</span></div></article>)}{assets.length === 0 ? <p className="text-sm text-muted">Nenhum arquivo enviado.</p> : null}</div></details>
  </section>;
}
