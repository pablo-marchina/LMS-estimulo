...
export default async function AdminReportsPage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  if (!organization?.permissions.includes("reporting.read")) return <AppShell area="admin" email={auth.email}><StatusPanel title="Relatórios restritos" tone="warning">Seu papel não permite consultar indicadores.</StatusPanel></AppShell>;
  ...
  return <AppShell area="admin" email={auth.email}><div className="grid gap-7">
    <PageHeader eyebrow="Evidência operacional" title="Relatórios" description="Indicadores reais da plataforma Estímulo, sem registros técnicos aposentados." />
