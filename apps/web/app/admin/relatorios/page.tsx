...
export default async function AdminReportsPage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  return <AppShell area="admin" email={auth.email}><div className="grid min-w-0 gap-6">
    ...
    {view === "aulas" ? <Card aria-labelledby="desempenho-aulas-titulo"><CardHeader><CardTitle id="desempenho-aulas-titulo">Engajamento por aula</CardTitle></CardHeader>{report.lessons.length === 0 ? <p className="text-sm text-muted">Ainda não há aulas disponibilizadas para participantes reais.</p> : <TableScroll><Table><thead><tr><Th>Aula</Th><Th>Tipo</Th><Th>Disponibilizadas</Th><Th>Iniciadas</Th><Th>Concluídas</Th><Th>Conclusão</Th></tr></thead><tbody>{report.lessons.map((item) => <tr key={item.activity_version_id}><Td className="font-medium">{item.title}</Td><Td>{item.activity_type}</Td><Td>{item.assigned}</Td><Td>{item.started}</Td><Td>{item.completed}</Td><Td>{item.completion_rate}%</Td></tr>)}</tbody></Table></TableScroll>}</Card> : null}
