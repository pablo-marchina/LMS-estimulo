...
function promptLibrary(configuration: Record<string, unknown>): PromptItem[] { return Array.isArray(configuration.prompts) ? configuration.prompts.map((item) => { const value = recordValue(item); return { title: stringValue(value.title), text: stringValue(value.text) }; }).filter((item) => item.title || item.text) : []; }
...
function aulaSummary(aula: EditableTrilhaAula) { const details: string[] = []; if (aula.assets.length) details.push(`${aula.assets.length} conteúdo(s)`); if (promptLibrary(aula.configuration ?? {}).length) details.push(`${promptLibrary(aula.configuration ?? {}).length} prompt(s)`); if (aula.assessment) details.push(`${aula.assessment.questions.length} pergunta(s)`); if (aula.practice) details.push("entrega prática"); return details.length ? details.join(" · ") : "Aula simples"; }
...
function AulaForm({ journeyVersionId, organizationId, pathTemplateId, libraryItems, aula, position }: { journeyVersionId: string; organizationId: string; pathTemplateId: string; libraryItems: ActivityLibraryOption[]; aula?: EditableTrilhaAula; position: number }) {
  ...
  const promptItems = promptLibrary(configuration);
  ...
  const hasOptionalResources = promptItems.length > 0 || questions.length > 0 || isPractice;
  ...
  return (
    <form action={saveAulaAction} encType="multipart/form-data" className="grid gap-5 border-t border-border p-4">
      ...
      <details className="rounded-2xl border border-primary/20 bg-primary-soft/20" open={hasOptionalResources}>
        <summary className="cursor-pointer px-4 py-3 text-sm font-black text-secondary">Recursos opcionais <span className="font-normal text-muted">· prompts, perguntas e entrega</span></summary>
        <div className="grid gap-4 border-t border-border p-4">
          <details className="rounded-xl border border-border bg-white" open={promptItems.length > 0}>
            <summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-secondary">Prompts prontos</summary>
            <div className="grid gap-3 border-t border-border p-4">
              <p className="text-xs leading-5 text-muted">Cadastre até 6 prompts reutilizáveis. Eles aparecem para o participante na “Biblioteca de prompts desta aula”, com ação de copiar.</p>
              {[0, 1, 2, 3, 4, 5].map((index) => (
                <div key={index} className="grid gap-2 rounded-xl bg-surface-muted p-3 sm:grid-cols-[.7fr_1.3fr]">
                  <Input name={`prompt_title_${index}`} defaultValue={promptItems[index]?.title ?? ""} placeholder={`Nome ${index + 1}`} />
                  <Textarea name={`prompt_text_${index}`} defaultValue={promptItems[index]?.text ?? ""} rows={3} placeholder="Prompt" />
                </div>
              ))}
            </div>
          </details>
