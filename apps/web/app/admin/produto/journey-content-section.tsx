...
import {
  TrilhaAulaBuilder,
  type EditableLessonTrack,
} from "./trilha-aula-builder";
import { TrilhaEditor, type EditableTrilha } from "./trilha-editor";

export function JourneyContentSection({
  model,
  organizationId,
}: {
  ...
  if (model.etapa !== "conteudo") return null;
  ...
  if (!selectedJourneyVersion) {
    return (
      <StatusPanel title="Salve as informações primeiro" tone="info">
        <ButtonLink href="/admin/produto?etapa=geral" className="mt-3 w-fit">
          Criar jornada
        </ButtonLink>
      </StatusPanel>
    );
    ...
    <div className="grid gap-5">
      <Card>
        ...
        <p className="mt-1 text-sm text-muted">
          Cada trilha reúne suas próprias aulas. Ao abrir uma aula, o conteúdo
          atual aparece antes da escolha de origem.
        </p>
      </Card>
      ...
      {selectedTrilhas.map((trilha) => (
        <section key={trilha.id} className="grid gap-3">
          <TrilhaEditor
            ...
            trilha={trilha as EditableTrilha}
          />
          <TrilhaAulaBuilder
            journeyVersionId={String(selectedJourneyVersion.id)}
            organizationId={organizationId}
            trilha={trilha as EditableLessonTrack}
      ...
      {selectedTrilhas.length === 0 ? (
        <StatusPanel title="Comece pela primeira trilha" tone="info">
          Uma trilha é um grupo de aulas sobre o mesmo objetivo.
        </StatusPanel>
      ) : null}

      <details className="rounded-2xl border border-primary/20 bg-white">
        <summary className="cursor-pointer px-5 py-4 font-semibold text-primary">
          Adicionar trilha
        </summary>
        <form
          action={saveTrilhaAction}
        ...
        >
          ...
          <label className="grid gap-1 text-sm font-medium text-ink">
            ...
            <span className="text-[11px] font-normal text-muted">
              Ex.: Primeiros passos, Vendas ou Gestão financeira.
            </span>
          </label>
          <label className="grid gap-1 text-sm font-medium text-ink">
            Explicação curta
          ...
          <input
            type="hidden"
            name="position"
            value={String(selectedTrilhas.length + 1)}
          />
