...
export function ActivityContentFields({
  ...
  const initialSource = currentLibraryItemVersionId ? "library" : currentAssetPresent ? "current" : "none";
  ...
  return (
    <section className="grid gap-3 rounded-2xl border border-primary/15 bg-primary-soft/30 p-4">
      <div><h3 className="font-black text-secondary">Conteúdo principal</h3><p className="text-[11px] text-muted">Mantenha, substitua ou remova o material desta aula.</p></div>
      <label className="grid gap-1 text-sm font-medium text-ink">Origem
        <Select name="content_source" value={source} onChange={(event) => setSource(event.target.value)}>
          {currentAssetPresent && !currentLibraryItemVersionId ? <option value="current">Manter conteúdo atual</option> : null}
