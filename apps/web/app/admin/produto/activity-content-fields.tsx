...
export function ActivityContentFields({ items, currentLibraryItemVersionId, currentAsset = null, currentContentRequired = false }: ActivityContentFieldsProps) {
  ...
  return (
    <section className="grid gap-3 rounded-2xl border border-primary/15 bg-primary-soft/30 p-4">
      <div><h3 className="font-black text-secondary">Conteúdo principal</h3><p className="text-[11px] text-muted">A Biblioteca é a fonte canônica. Quando o conteúdo estiver vinculado, edite-o nela para manter todas as aulas sincronizadas.</p></div>
      ...
      {currentAsset ? <div className="grid gap-3 rounded-xl border border-border bg-white p-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {currentLibraryItemVersionId ? <a href={`/admin/biblioteca?view=novo&edit=${encodeURIComponent(currentLibraryItemVersionId)}`} className="inline-flex w-fit items-center gap-1.5 rounded-lg border border-primary/20 bg-primary-soft px-2.5 py-1.5 text-xs font-bold text-primary hover:bg-primary hover:text-white"><ExternalLink size={13} />Editar na Biblioteca</a> : null}
        </div>
        {!currentLibraryItemVersionId ? <div className="grid gap-3 border-t border-border pt-3"><p className="text-xs font-bold uppercase tracking-[.1em] text-primary">Texto exibido ao participante</p><label className="grid gap-1 text-sm font-medium text-ink">Título do conteúdo<Input value={assetTitle} onChange={(event) => { setAssetTitle(event.target.value); setAssetSaveState("idle"); }} maxLength={240} /></label><label className="grid gap-1 text-sm font-medium text-ink">Descrição<Textarea value={assetDescription} onChange={(event) => { setAssetDescription(event.target.value); setAssetSaveState("idle"); }} rows={3} maxLength={2000} /><span className="text-[11px] font-normal text-muted">Este é o texto que aparece acima do player. Pode ser alterado sem recriar o conteúdo.</span></label><div className="flex flex-wrap items-center gap-3"><button type="button" onClick={saveCurrentAssetCopy} disabled={assetSaveState === "saving" || !assetTitle.trim()} className="inline-flex items-center gap-2 rounded-xl bg-primary px-3 py-2 text-sm font-bold text-white transition hover:bg-primary-active disabled:cursor-not-allowed disabled:opacity-60"><Save size={15} />{assetSaveState === "saving" ? "Salvando…" : "Salvar texto do conteúdo"}</button>{assetSaveState === "saved" ? <span className="text-xs font-bold text-success">Texto salvo.</span> : null}{assetSaveState === "error" ? <span className="text-xs font-bold text-danger">Não foi possível salvar. Tente novamente.</span> : null}</div></div> : null}
      </div> : null}

      <label className="grid gap-1 text-sm font-medium text-ink">Origem<Select name="content_source" value={source} onChange={(event) => setSource(event.target.value)}>{currentAsset && !currentLibraryItemVersionId ? <option value="current">Manter conteúdo atual</option> : null}<option value="none">Sem conteúdo principal</option><option value="library">Usar da Biblioteca</option><option value="new">Criar agora na Biblioteca</option></Select><span className="text-[11px] font-normal text-muted">Escolher “sem conteúdo” remove o material quando a aula for salva.</span></label>

      {source === "library" ? <div className="grid gap-3"><label className="grid gap-1 text-sm font-medium text-ink">Material<Select name="library_item_version_id" value={selectedLibraryItemVersionId} onChange={(event) => setSelectedLibraryItemVersionId(event.target.value)} required><option value="">Selecione</option>{items.map((item) => <option key={item.library_item_version_id} value={item.library_item_version_id}>{item.title} · {item.content_format}</option>)}</Select><span className="text-[11px] font-normal text-muted">Somente conteúdos publicados.</span></label>{selectedLibraryItem ? <div className="grid gap-2 rounded-xl bg-white p-3"><strong className="text-sm text-secondary">{selectedLibraryItem.title}</strong>{selectedLibraryItem.summary ? <p className="text-xs leading-5 text-muted">{selectedLibraryItem.summary}</p> : null}{selectedLibraryItem.body ? <p className="max-h-40 overflow-y-auto whitespace-pre-line rounded-lg bg-surface-muted p-3 text-xs leading-5 text-muted">{selectedLibraryItem.body}</p> : null}<div className="flex flex-wrap gap-3">{selectedLibraryItem.external_url ? <a href={selectedLibraryItem.external_url} target="_blank" rel="noreferrer" className="inline-flex w-fit items-center gap-1.5 text-xs font-bold text-primary hover:underline"><ExternalLink size={13} />Abrir material</a> : selectedLibraryItem.original_filename ? <p className="text-xs font-semibold text-primary">Arquivo: {selectedLibraryItem.original_filename}</p> : null}<a href={`/admin/biblioteca?view=novo&edit=${encodeURIComponent(selectedLibraryItem.library_item_version_id)}`} className="inline-flex w-fit items-center gap-1.5 text-xs font-bold text-primary hover:underline"><ExternalLink size={13} />Editar este conteúdo na Biblioteca</a></div></div> : null}</div> : null}
