...
export default async function AdminProductPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const errorMessage = error === "campos_incompletos"
    ? "A aula de encerramento precisa de título, quiz válido e checklist da entrega prática."
    : "Revise os campos, as permissões e os dados informados.";
  ...
  return <AppShell area="admin" email={auth.email}>
    <div className="grid gap-8">
      <PageHeader
        ...
        description="Crie a jornada, organize suas trilhas e adicione aulas no mesmo fluxo. Versões publicadas permanecem imutáveis."
        actions={
      ...
      {error ? <StatusPanel title="Não foi possível salvar" tone="warning"><p>{errorMessage}</p></StatusPanel> : null}
      ...
      <section className="grid gap-4">
        <details className="group rounded-xl border border-border bg-surface" open>
          <summary className="flex cursor-pointer items-center justify-between gap-4 p-5 marker:content-none [&::-webkit-details-marker]:hidden">
            <span className="grid gap-1">
              ...
              <small className="text-muted">Dados, acesso, trilhas e aulas</small>
            </span>
          ...
          <div className="grid gap-6 border-t border-border p-5">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <label className="grid gap-1.5 text-sm font-medium text-ink">Definição existente
                  <Select name="definition_id" defaultValue={selectedJourneyVersion?.definitionId ?? ""}>
                    <option value="">Nova jornada</option>
            ...
            <section className="grid gap-3" aria-labelledby="trilhas-heading">
              ...
              ) : <p className="rounded-lg border border-dashed border-border p-4 text-sm text-muted">Selecione uma jornada draft para organizar trilhas e aulas.</p>}
            </section>
            ...
            <fieldset className="grid gap-3 rounded-xl border border-border p-4">
              ...
              ) : <p className="text-sm text-muted">Salve ou selecione uma jornada draft antes de adicionar trilhas.</p>}
            </fieldset>
          </div>
        </details>
        ...
        <details className="group rounded-xl border border-border bg-surface">
          <summary className="flex cursor-pointer items-center justify-between gap-4 p-5 marker:content-none [&::-webkit-details-marker]:hidden">
            <span className="grid gap-1">
              <strong className="text-ink">Regra avançada</strong>
