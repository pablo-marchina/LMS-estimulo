...
export default async function AdminProductPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const successTitle = success === "jornada_publicada" ? "Jornada publicada" : success === "atualizado_ao_vivo" ? "Atualização publicada" : success === "jornada_em_rascunho" ? "Jornada voltou para rascunho" : success === "rascunho_excluido" ? "Rascunho excluído" : "Rascunho salvo";
  const successMessage = success === "atualizado_ao_vivo" ? "As mudanças já estão disponíveis para participantes no próximo carregamento." : success === "jornada_publicada" ? "A jornada já pode ser acessada pelos participantes." : success === "jornada_em_rascunho" ? "A publicação anterior foi retirada e uma nova versão editável foi criada, preservando todo o histórico." : success === "rascunho_excluido" ? "O rascunho deixou de aparecer na administração. Versões publicadas e históricos foram preservados." : "Continue a edição ou publique quando estiver pronto.";
  ...
  return <AppShell area="admin" email={auth.email}><div className="grid gap-6">
    ...
    {error ? <StatusPanel title="Não foi possível concluir" tone="warning">{error === "somente_rascunho" ? "Somente rascunhos podem ser excluídos." : error === "publicacao_nao_encontrada" ? "A jornada já não está publicada ou foi alterada por outra pessoa." : "Revise os campos obrigatórios e tente novamente."}</StatusPanel> : null}
    ...
    <fieldset disabled={!canEditSelected} className="contents">
      ...
      </form>{selectedIsDraft && selectedJourneyVersion && canEdit ? <details className="rounded-2xl border border-danger/25 bg-danger/5"><summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-danger">Excluir rascunho</summary><form action={deleteJourneyAction} className="grid gap-3 border-t border-danger/20 p-4"><input type="hidden" name="journey_version_id" value={String(selectedJourneyVersion.id)} /><p className="text-sm text-muted">Este rascunho deixará de aparecer na administração. Uma versão publicada anterior, quando existir, continuará preservada.</p><label className="flex items-start gap-2 text-sm text-ink"><input type="checkbox" name="confirm_delete" value="true" required className="mt-0.5 size-4 accent-danger" />Confirmo que desejo excluir somente este rascunho.</label><Button type="submit" variant="danger" size="sm" className="w-fit">Excluir rascunho</Button></form></details> : null}</Card> : null}
