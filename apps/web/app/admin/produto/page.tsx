...
export default async function AdminProductPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const latestVersionIds = new Set(activeJourneys.map((journey) => journey.versions.filter((version) => version.status === "draft" || version.status === "published").slice().sort((a, b) => b.version_number - a.version_number)[0]?.id).filter(Boolean).map(String));
  ...
  const configuredCompletionCertificateVersionId = stringValue(completionCertificate.certificate_version_id);
  const completionCertificateOptions = selectedJourneyVersion
    ? workspace.certificates
        ...
        .flatMap((definition) => definition.versions
          .filter((version) => version.status === "published" && String(version.journey_version_id ?? "") === String(selectedJourneyVersion.id))
          .map((version) => ({ id: String(version.id), name: definition.name, versionNumber: Number(version.version_number) })))
        .sort((a, b) => a.name.localeCompare(b.name, "pt-BR") || b.versionNumber - a.versionNumber)
    : [];
  const presentationTags = Array.isArray(presentation.tags) ? presentation.tags.filter((item): item is string => typeof item === "string") : [];
  ...
  const selectedThemeIds = managedThemes.filter((theme) => presentationTags.includes(theme.name)).map((theme) => theme.id);
  ...
  const successTitle = success === "jornada_publicada" ? "Jornada publicada" : success === "jornada_arquivada" ? "Jornada arquivada" : success === "atualizado_ao_vivo" ? "Atualização publicada" : success === "jornada_em_rascunho" ? "Jornada voltou para rascunho" : success === "rascunho_excluido" ? "Rascunho excluído" : "Rascunho salvo";
  const successMessage = success === "jornada_arquivada" ? "A jornada saiu das listas ativas e o histórico foi preservado." : success === "atualizado_ao_vivo" ? "As mudanças já estão disponíveis para participantes no próximo carregamento." : success === "jornada_publicada" ? "A jornada já pode ser acessada pelos participantes." : success === "jornada_em_rascunho" ? "A jornada foi retirada do ar e voltou ao estado de rascunho. Participantes em andamento foram interrompidos." : success === "rascunho_excluido" ? "A jornada em rascunho foi removida da administração." : "Continue a edição ou publique quando estiver pronto.";
  const errorMessage = error === "certificado_conclusao_obrigatorio" ? "Selecione qual certificado deve ser emitido ao concluir a jornada." : error === "certificado_conclusao_incompativel" ? "O certificado selecionado não está publicado ou não pertence a esta jornada." : error === "somente_rascunho" ? "Somente rascunhos podem ser excluídos." : error === "publicacao_nao_encontrada" ? "A jornada já não está publicada ou foi alterada por outra pessoa." : error === "confirmacao_arquivamento" ? "Digite ARQUIVAR para confirmar o arquivamento da jornada." : error === "jornada_nao_encontrada" ? "A jornada não foi encontrada ou já foi arquivada." : error === "falha_arquivamento" ? "Não foi possível arquivar a jornada. Tente novamente ou revise as permissões." : "Revise os campos obrigatórios e tente novamente.";
  ...
  return <AppShell area="admin" email={auth.email}><div className="grid gap-6">
    ...
    {selectedIsPublished ? <StatusPanel title="Jornada publicada" tone="warning">Salvar altera a experiência dos participantes no próximo carregamento. Para retirar a jornada do ar e continuar trabalhando, use “Voltar para rascunho” na etapa Publicação.</StatusPanel> : null}
    ...
    <fieldset disabled={!canEditSelected} className="contents">
      {etapa === "geral" ? <Card className="grid gap-5"><div><h2 className="text-lg font-black text-secondary">Informações principais</h2><p className="mt-1 text-sm text-muted">Título é o único campo obrigatório para criar uma jornada.</p></div><form className="grid gap-4" action={saveJourneyAction} encType="multipart/form-data">
        ...
        <details className="rounded-2xl border border-border" open={completionCertificateEnabled}><summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-secondary">Conclusão e certificado</summary><div className="grid gap-4 border-t border-border p-4"><p className="text-xs text-muted">A jornada decide se a conclusão gera certificado. O catálogo de certificados continua responsável pelo template, validade e identidade visual.</p>{selectedJourneyVersion ? <><label className="flex items-start gap-3 rounded-xl bg-primary-soft p-3 text-sm text-ink"><input type="checkbox" name="completion_certificate_enabled" defaultChecked={completionCertificateEnabled} className="mt-0.5 size-4 accent-primary" /><span><strong className="block">Emitir certificado ao concluir a jornada</strong><small className="text-muted">A emissão acontece automaticamente quando o estado muda para concluída.</small></span></label><label className="grid gap-1 text-sm font-medium text-ink">Certificado emitido<Select name="completion_certificate_version_id" defaultValue={configuredCompletionCertificateVersionId}><option value="">Selecione um certificado publicado</option>{completionCertificateOptions.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</Select><span className="text-[11px] font-normal text-muted">Somente certificados publicados e já vinculados a esta jornada aparecem aqui.</span></label>{completionCertificateOptions.length === 0 ? <div className="grid gap-3 rounded-xl border border-warning/30 bg-warning-soft p-3"><p className="text-sm text-ink">Nenhum certificado publicado está disponível para esta jornada. Configure o certificado antes de ativar a emissão.</p><ButtonLink href="/admin/gamificacao?tipo=certificados" variant="secondary" size="sm" className="w-fit">Configurar certificados</ButtonLink></div> : null}<div className="rounded-xl bg-surface-muted p-3"><strong className="text-sm text-secondary">Dados preenchidos automaticamente</strong><p className="mt-1 text-xs text-muted">Nome do participante, título da jornada, data de emissão e código de verificação. Gatilho canônico: <code>journey.instance.completed</code>.</p></div></> : <p className="text-sm text-muted">Salve a jornada primeiro. Depois você poderá associar um certificado publicado a ela.</p>}</div></details>
        <Button type="submit" className="w-fit">{selectedIsPublished ? "Salvar e atualizar agora" : "Salvar rascunho"}</Button>
      </form>{selectedIsDraft && selectedJourneyVersion && canEdit ? <details className="rounded-2xl border border-danger/25 bg-danger/5"><summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-danger">Excluir rascunho</summary><form action={deleteJourneyAction} className="grid gap-3 border-t border-danger/20 p-4"><input type="hidden" name="journey_version_id" value={String(selectedJourneyVersion.id)} /><p className="text-sm text-muted">A jornada será removida da administração. Esta ação não é permitida enquanto ela estiver publicada.</p><label className="flex items-start gap-2 text-sm text-ink"><input type="checkbox" name="confirm_delete" value="true" required className="mt-0.5 size-4 accent-danger" />Confirmo que desejo excluir esta jornada em rascunho.</label><Button type="submit" variant="danger" size="sm" className="w-fit">Excluir rascunho</Button></form></details> : null}</Card> : null}
      ...
      {etapa === "publicacao" ? selectedJourneyVersion ? <Card className="grid gap-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-black text-secondary">Publicação</h2><p className="mt-1 text-sm text-muted">Confira a estrutura antes de disponibilizar a jornada.</p></div><StatusPill tone={selectedIsPublished ? "success" : "neutral"}>{versionStatus(selectedJourneyVersion.status)}</StatusPill></div><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-primary-soft p-4"><strong className="text-2xl text-primary">{selectedTrilhas.length}</strong><p className="text-sm text-muted">trilhas</p></div><div className="rounded-xl bg-success-soft p-4"><strong className="text-2xl text-success">{selectedTrilhas.reduce((sum, item) => sum + item.aulas.length, 0)}</strong><p className="text-sm text-muted">aulas</p></div><div className="rounded-xl bg-warning-soft p-4"><strong className="text-2xl text-warning">{selectedTrilhas.filter((item) => item.aulas.length === 0).length}</strong><p className="text-sm text-muted">trilhas sem aula</p></div></div>{selectedIsDraft && canPublish ? <form action={publishJourneyAction}><input type="hidden" name="journey_version_id" value={String(selectedJourneyVersion.id)} /><input type="hidden" name="content_hash" value={selectedJourneyVersion.content_hash ?? ""} /><Button type="submit" disabled={!graphLooksComplete || !selectedJourneyVersion.content_hash}>Publicar jornada</Button>{!graphLooksComplete ? <p className="mt-2 text-sm text-warning">Cada trilha precisa ter ao menos uma aula.</p> : null}</form> : selectedIsPublished ? <div className="grid gap-4"><StatusPanel title="Publicação ativa" tone="success">A jornada está no ar. Alterações salvas continuam sendo aplicadas imediatamente.</StatusPanel>{canPublish ? <><details className="rounded-2xl border border-warning/30 bg-warning-soft/40"><summary className="cursor-pointer px-4 py-3 text-sm font-bold text-warning">Voltar para rascunho</summary><form action={unpublishJourneyAction} className="grid gap-3 border-t border-warning/25 p-4"><input type="hidden" name="journey_version_id" value={String(selectedJourneyVersion.id)} /><p className="text-sm text-muted">A jornada será retirada do catálogo e voltará ao estado de rascunho. Participantes em andamento serão interrompidos imediatamente.</p><label className="flex items-start gap-2 text-sm text-ink"><input type="checkbox" name="confirm_unpublish" value="true" required className="mt-0.5 size-4 accent-warning" />Confirmo que desejo retirar esta jornada do ar e interromper os acessos em andamento.</label><Button type="submit" variant="secondary" size="sm" className="w-fit">Voltar para rascunho</Button></form></details><details className="rounded-2xl border border-danger/25 bg-danger/5"><summary className="cursor-pointer px-4 py-3 text-sm font-bold text-danger">Arquivar jornada</summary><form action={retireJourneyAction} className="grid gap-3 border-t border-danger/20 p-4"><input type="hidden" name="journey_definition_id" value={String(selectedJourneyVersion.definitionId)} /><p className="text-sm text-muted">Arquivar remove a jornada das listas ativas sem apagar o histórico. Use “Voltar para rascunho” se sua intenção for interromper imediatamente participantes em andamento e continuar editando.</p><label className="grid gap-1 text-sm font-medium text-ink">Digite ARQUIVAR<Input name="confirmation" autoComplete="off" required /></label><Button type="submit" variant="danger" size="sm" className="w-fit">Confirmar arquivamento</Button></form></details></> : null}</div> : <p className="text-sm text-muted">Somente um administrador com permissão de publicação pode concluir esta etapa.</p>}</Card> : <StatusPanel title="Escolha uma jornada" tone="info">Abra uma jornada para revisar sua publicação.</StatusPanel> : null}
    </fieldset>
  </div></AppShell>;
}
