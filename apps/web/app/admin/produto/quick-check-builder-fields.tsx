  ...
  return (
    <details className="rounded-2xl border border-border bg-white" open={questions.length > 0}>
      ...
      <div className="grid gap-4 border-t border-border p-4">
        <div className="flex flex-wrap items-center justify-between gap-3"><p className="max-w-2xl text-xs text-muted">Adicione quantas perguntas forem necessárias. Respostas abertas não são corrigidas: qualquer texto preenchido conta como participação válida.</p><div className="flex gap-2"><Button type="button" variant="secondary" size="sm" onClick={removeQuestion} disabled={count === 0}>Remover última</Button><Button type="button" variant="secondary" size="sm" onClick={addQuestion}>Adicionar pergunta</Button></div></div>
