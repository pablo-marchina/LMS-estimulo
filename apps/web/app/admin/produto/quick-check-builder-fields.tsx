...
export function QuickCheckBuilderFields({ questions = [] }: { questions?: ExistingQuickQuestion[] }) {
  ...
  return (
    <details className="rounded-2xl border border-border bg-white">
      ...
      <div className="grid gap-4 border-t border-border p-4">
        ...
        {count > 0 ? <div className="grid gap-3 sm:grid-cols-2"><label className="grid gap-1 text-sm font-medium text-ink">Aprovação (%)<Input name="quiz_passing_score" type="number" min="0" max="100" defaultValue="70" /><span className="text-[11px] font-normal text-muted">Ignorado quando todas as perguntas são abertas.</span></label><label className="grid gap-1 text-sm font-medium text-ink">Tentativas<Input name="quiz_max_attempts" type="number" min="1" max="20" defaultValue="3" /></label></div> : null}
      </div>
    </details>
  );
}
