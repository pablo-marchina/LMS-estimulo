    ...
    <div className="grid gap-8">
      ...
      {workspace ? <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          {workspace.journey_versions.filter((item) => item.status === "draft").length === 0 ? <p className="mt-2 text-sm text-muted">Nenhuma versão em rascunho está pronta.</p> : <form action={publishVerticalAction} className="mt-4 grid gap-4">
        ...
        <Card>
          ...
          {workspace.participants.length === 0 || workspace.journey_versions.filter((item) => item.status === "published").length === 0 ? <p className="mt-2 text-sm text-muted">É necessário ter participante técnico e versão publicada.</p> : <form action={createEnrollmentAction} className="mt-4 grid gap-4">
            <input type="hidden" name="organization_id" value={organization.organization_id} />
            <input type="hidden" name="idempotency_key" value={randomUUID()} />
            <label className="grid gap-1.5 text-sm font-medium text-ink">Participante
              <Select name="entrepreneur_id" required>
                {workspace.participants.map((item) => <option key={item.entrepreneur_id} value={item.entrepreneur_id}>{item.display_name} · {item.email}</option>)}
              </Select>
            </label>
            <label className="grid gap-1.5 text-sm font-medium text-ink">Jornada
              <Select name="journey_version_id" required>
                {workspace.journey_versions.filter((item) => item.status === "published").map((item) => <option key={item.journey_version_id} value={item.journey_version_id}>{item.title} · versão {item.version_number}</option>)}
              </Select>
            </label>
      ...
      {canReviewPractice ? <section className="grid gap-4" id="praticas" aria-labelledby="revisao-praticas-titulo">
        <div>
          <h2 id="revisao-praticas-titulo" className="text-xl font-semibold text-ink">Revisão de práticas</h2>
          <p className="text-sm text-muted">Arquivos privados validados por autorização, formato, tamanho e hash podem ser baixados e avaliados.</p>
        </div>
        {practices.length === 0 ? <EmptyState title="Nenhuma prática" tone="info">Ainda não há evidências enviadas nesta organização.</EmptyState> : <div className="grid gap-4">{practices.map((practice) => <Card key={practice.id}>
          <div className="mb-2 flex flex-wrap items-center gap-2">
            <strong className="text-ink">{practice.participant_name}</strong>
            <StatusPill tone={practiceTone(practice.status)}>{practiceStatus[practice.status] ?? practice.status}</StatusPill>
            <time dateTime={practice.submitted_at} className="ml-auto text-xs text-muted">{dateFormatter.format(new Date(practice.submitted_at))}</time>
          </div>
          ...
          <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted">
            <span>{practice.content_type ?? "Formato em validação"}</span>
            {fileSize(practice.size_bytes) ? <span>{fileSize(practice.size_bytes)}</span> : null}
            <span>{practice.allow_public_use ? "Uso autorizado" : "Uso público não autorizado"}</span>
          </div>
          ...
          {practice.status === "awaiting_review" ? <div className="mt-4 grid gap-3 border-t border-border pt-4 sm:grid-cols-2">
            <form action={reviewPracticeSubmissionAction} className="grid gap-3">
              <input type="hidden" name="organization_id" value={organization.organization_id} />
              <input type="hidden" name="submission_id" value={practice.id} />
              <input type="hidden" name="status" value="accepted" />
              <input type="hidden" name="idempotency_key" value={randomUUID()} />
              <label className="grid gap-1.5 text-sm font-medium text-ink">Feedback opcional
                <Textarea name="feedback" rows={2} maxLength={2000} placeholder="Registre uma orientação para o participante." />
              </label>
              <Button type="submit" className="w-fit">Aceitar prática</Button>
            </form>
            <form action={reviewPracticeSubmissionAction} className="grid gap-3">
              <input type="hidden" name="organization_id" value={organization.organization_id} />
              <input type="hidden" name="submission_id" value={practice.id} />
              ...
              <input type="hidden" name="idempotency_key" value={randomUUID()} />
              <label className="grid gap-1.5 text-sm font-medium text-ink">Motivo da revisão
                <Textarea name="feedback" rows={2} minLength={1} maxLength={2000} required placeholder="Explique o que precisa ser ajustado." />
              </label>
              <Button variant="secondary" type="submit" className="w-fit">Solicitar ajuste</Button>
            </form>
          </div> : null}
        </Card>)}</div>}
      </section> : null}

      {canManageComments ? <section className="grid gap-4" id="comentarios" aria-labelledby="moderacao-comentarios-titulo">
        <div>
          <h2 id="moderacao-comentarios-titulo" className="text-xl font-semibold text-ink">Moderação de comentários</h2>
          <p className="text-sm text-muted">Comentários visíveis podem ser ocultados com justificativa. Comentários ocultos podem ser restaurados sem apagar o histórico.</p>
        </div>
        {comments.length === 0 ? <EmptyState title="Nenhum comentário" tone="info">Ainda não há comentários para moderar nesta organização.</EmptyState> : <div className="grid gap-4">{comments.map((comment) => <Card key={comment.id}>
          <div className="mb-2 flex flex-wrap items-center gap-2">
            <strong className="text-ink">{comment.author_name}</strong>
            <StatusPill tone={comment.status === "visible" ? "success" : "neutral"}>{comment.status === "visible" ? "Visível" : "Oculto"}</StatusPill>
            <time dateTime={comment.created_at} className="ml-auto text-xs text-muted">{dateFormatter.format(new Date(comment.created_at))}</time>
          </div>
          <p className="text-sm text-muted">{comment.activity_title}</p>
          ...
          {comment.status === "visible" ? <form action={moderateActivityCommentAction} className="mt-4 grid gap-3 border-t border-border pt-4">
            <input type="hidden" name="organization_id" value={organization.organization_id} />
            <input type="hidden" name="comment_id" value={comment.id} />
            <input type="hidden" name="status" value="hidden" />
            <input type="hidden" name="idempotency_key" value={randomUUID()} />
            <label className="grid gap-1.5 text-sm font-medium text-ink">Justificativa
              <Textarea name="reason" rows={2} minLength={1} maxLength={500} required placeholder="Explique por que o comentário deve ser ocultado." />
            </label>
            <Button variant="secondary" type="submit" className="w-fit">Ocultar comentário</Button>
          </form> : <form action={moderateActivityCommentAction} className="mt-4 border-t border-border pt-4">
            <input type="hidden" name="organization_id" value={organization.organization_id} />
            <input type="hidden" name="comment_id" value={comment.id} />
            <input type="hidden" name="status" value="visible" />
            <input type="hidden" name="reason" value="" />
            <input type="hidden" name="idempotency_key" value={randomUUID()} />
            <Button variant="secondary" type="submit" className="w-fit">Restaurar comentário</Button>
          </form>}
        </Card>)}</div>}
      </section> : null}
      ...
      <section className="grid gap-4">
        ...
        {listing.instances.length === 0 ? <EmptyState title="Nenhuma instância" tone="info">A organização ainda não possui jornadas executadas.</EmptyState> : <div className="grid gap-4">{listing.instances.map((instance) => <Card key={instance.journey_instance_id}>
          <div className="mb-3 flex flex-wrap items-center gap-2">
            <StatusPill tone="info">{statusLabel(instance.journey_status)}</StatusPill>
            <span className="text-sm text-muted">{instance.journey_code}</span>
          </div>
          <ProgressMeter value={instance.progress} label="Progresso" />
          <ButtonLink href={`/admin?organization=${organization.organization_id}&instance=${instance.journey_instance_id}`} variant="secondary" size="sm" className="mt-4 w-fit">Abrir evidências</ButtonLink>
        </Card>)}</div>}
      </section>

      {result ? <Card>
        <h2 className="text-lg font-semibold text-ink">Evidência selecionada</h2>
        <pre className="mt-3 overflow-auto rounded-lg bg-ink p-4 font-mono text-xs text-white">{JSON.stringify(result, null, 2)}</pre>
      </Card> : null}
    </div>
  </AppShell>;
}
