...
async function AnnouncementList({ actor, organizationId, canEdit }: { actor: string; organizationId: string; canEdit: boolean }) {
  ...
  return <section className="grid min-w-0 gap-4" aria-labelledby="anuncios-existentes-titulo">
    ...
    <div className="grid min-w-0 gap-4">{data.announcements.map((announcement) => <details className="min-w-0 max-w-full overflow-hidden rounded-xl border border-border bg-white" key={announcement.id}>
      <summary className="flex min-w-0 cursor-pointer list-none flex-wrap items-center gap-4 p-5 sm:flex-nowrap">
        {announcement.image_file_object_id ? <picture className="shrink-0"><source media="(max-width: 639px)" srcSet={`/api/announcements/${announcement.id}/image?variant=mobile`} /><img src={`/api/announcements/${announcement.id}/image`} alt="" className="h-16 w-28 rounded-lg border border-border object-cover" /></picture> : <span className="grid h-16 w-28 shrink-0 place-items-center rounded-lg bg-surface-muted text-muted"><ImageIcon /></span>}
